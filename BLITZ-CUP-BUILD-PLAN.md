...
- **Excluded:** any SAST wall-clock cutoff/scheduling helper (e.g. `nextSastHourBoundary`, `weekendWindow`-style timing) — Blitz Cup uses its own 1hr lobby timer + 4hr auto-finish timer instead, not day-of-week/timezone cutoffs.
...
- Direct port of the League Ladder comments component onto the Blitz Cup league page.
...
- Prefilled message template includes: league type ("Blitz Cup"), the player's name, and a direct link to that league.
...
- Hall of Fame leaderboard — all-time top Blitz Cup earners (v2/optional).
