...
export default function ChallengesScreen({ session, members, challenges, openChallenges, recentResults, boardComments, isAdmin, accounts, myUsername, onPostBoardComment, onDeleteBoardComment, onToggleBoardCommentReaction, onSendChallenge, onAccept, onDecline, onRemove, onOpenLogResult, onConfirmResult, onDisputeResult, onOpenLogResultOpen, onConfirmResultOpen, onDisputeResultOpen, onAdminApproveResult, onAdminRejectResult, onAdminApproveResultOpen, onAdminRejectResultOpen, onAdminEditResult, onAdminEditResultOpen, onAdminGrantLadderWalkover, onAdminCancelLadderChallenge, onViewResultProof, onSendRandom, onAcceptOpen, onCancelOpen, onRemoveOpen, onBack, showToast, c }) {
  ...
  //
  // escalatedChallenges/escalatedOpenChallenges above (the "opponent didn't
  // respond within the window" box) and isCommunityResultEscalated here are
  // both just "past the confirm window, still unconfirmed" — computed
  // independently off two different data sources (challenges/open_challenges
  // vs. the public_challenge_results view) with the same underlying
  // condition. Anything that satisfies both showed up twice — once with
  // full actions up top, once again down here. Excluded by id so each
  // escalated result appears in exactly one place.
  const timeoutEscalatedIds = new Set([
    ...escalatedChallenges.map((ch) => String(ch.id)),
    ...escalatedOpenChallenges.map((ch) => String(ch.id)),
  ]);
  const escalatedCommunityResults = isAdmin
    ? filteredResults.filter((r) => isCommunityResultEscalated(r) && !timeoutEscalatedIds.has(String(r.id)))
    : [];
...
function AdminEscalatedResultRow({ nameA, nameB, scoreA, scoreB, reportedByUsername, reporterPhone, onApprove, onReject, onEditScore, onViewProof, c }) {
  ...
  const saveEdit = async () => {
    ...
    // loser's 0. Only block when both are blank, or when what's typed
    // isn't a valid non-negative integer.
    if (editA === "" && editB === "") return;
    const a = editA === "" ? 0 : parseInt(editA, 10);
    const b = editB === "" ? 0 : parseInt(editB, 10);
