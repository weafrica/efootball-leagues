...
function generateOpeningFixtures(league, teamIds, dueBase) {
  const { id: leagueId, format, survivor_matches_per_stage, survivor_target_count, survivor_final_format, groups_count } = league;
  ...
  if (format === "knockout") return { fixtureRows: toFixtureRows(leagueId, [knockoutRound1(teamIds)], 1, dueBase), startsInFinal: false, groups: null };
  if (format === "survivor") {
  ...
  if (format === "groups_knockout") {
    const groupsCount = Math.max(2, Math.min(groups_count || 2, Math.floor(teamIds.length / 2)));
    ...
    return { fixtureRows, startsInFinal: false, groups };
  }
...
function knockoutBracketFixtures(leagueId, teamIds, roundOffset, dueBase) {
  return toFixtureRows(leagueId, [knockoutRound1(teamIds)], 2, dueBase, roundOffset);
}
...
export default function App() {
  ...
  const createLeague = async (input) => {
    const { name, teamNames, format, survivor, groups, entryClosesAt, startsAt, description, leagueType } = input;
    const insertPayload = {
    ...
    if (format === "groups_knockout") {
      insertPayload.groups_count = groups.groupsCount;
    ...
    if (teamNames.length >= 2) {
      ...
      const { fixtureRows, startsInFinal, groups: groupAssignments } = generateOpeningFixtures(league, teams.map((t) => t.id), generationDueBase(league));
      if (groupAssignments) { const ok = await persistGroupAssignments(groupAssignments); if (!ok) return; }
  ...
  const generateFixtures = async (league) => {
    ...
    if (league.format === "groups_knockout" && league.teams.length < league.groups_count * 2) {
      showToast(`Need at least ${league.groups_count * 2} clubs to fill ${league.groups_count} groups.`); return;
    }
    const { fixtureRows, startsInFinal, groups: groupAssignments } = generateOpeningFixtures(league, league.teams.map((t) => t.id), generationDueBase(league));
    if (groupAssignments) { const ok = await persistGroupAssignments(groupAssignments); if (!ok) return; }
    const ok = await insertChunked("fixtures", fixtureRows, showToast);
    if (!ok) return;
    if (startsInFinal) await supabase.from("leagues").update({ final_stage_started: true }).eq("id", league.id);
  ...
  const advanceGroupsToKnockout = async (league) => {
    ...
    const fixtureRows = knockoutBracketFixtures(league.id, shuffle(qualifiers), 0, new Date());
    const ok = await insertChunked("fixtures", fixtureRows, showToast);
    if (!ok) return;
  ...
  const recordResult = async (league, fixture, homeScore, awayScore) => {
    ...
    if (inKnockoutBracket && homeScore !== awayScore) {
  ...
  const advanceKnockout = async (league) => {
    ...
    const fixtureRows = toFixtureRows(league.id, [knockoutRound1(winners)], bracketStage, new Date(), maxRound);
    const ok = await insertChunked("fixtures", fixtureRows, showToast);
    if (!ok) return;
    await loadLeagues();
...
function CreateLeague({ onCancel, onCreate, isAdmin, c }) {
  ...
  const groupsValid = format !== "groups_knockout" || (groupsCount >= 2 && qualifiersPerGroup >= 1 && (teamNames.length === 0 || teamNames.length >= groupsCount * 2));
  const groupsTooFewTeams = format === "groups_knockout" && teamNames.length > 0 && teamNames.length < groupsCount * 2;
  ...
  const submit = () => {
    onCreate({
      ...
      groups: format === "groups_knockout" ? { groupsCount: Number(groupsCount), qualifiersPerGroup: Number(qualifiersPerGroup) } : null,
      ...
      {isAdmin && (
        <div className="mb-5">
          ...
          <div className="font-mono text-xs mt-1.5" style={{ color: c.textFaint }}>
            Cash leagues work exactly like fun leagues — it's just a label, shown to players so they know what's at stake.
          </div>
        </div>
      )}

      <label className="block font-mono text-xs uppercase tracking-wider mb-2" style={{ color: c.textDim }}>Description <span style={{ color: c.textFaint }}>(optional — rules, prize, WhatsApp group link, etc.)</span></label>
      <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="e.g. Winner takes the pot. Join the WhatsApp group: ..." className="w-full border rounded-lg px-4 py-2.5 font-body outline-none resize-none mb-5" style={inputStyle} />
      ...
      {format === "groups_knockout" && (
        <div className="rounded-lg p-4 border mb-5" style={{ background: c.surface, borderColor: c.border }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-2">
            <div>
              <label className="block font-body text-xs mb-1" style={{ color: c.textDim }}>Number of groups</label>
              <input type="number" min={2} value={groupsCount} onChange={(e) => setGroupsCount(e.target.value)} className="w-full border rounded-lg px-3 py-2 font-mono text-sm outline-none" style={inputStyle} />
            </div>
            <div>
          ...
          <div className="font-body text-xs" style={{ color: groupsTooFewTeams ? c.red : c.textFaint }}>
            {groupsTooFewTeams
              ? `Need at least ${groupsCount * 2} clubs to fill ${groupsCount} groups — add more or reduce the group count.`
              : `${groupsCount} group${groupsCount == 1 ? "" : "s"} · top ${qualifiersPerGroup} from each (${groupsCount * qualifiersPerGroup} total) advance to a single-elimination knockout. Clubs are split into groups once the league starts.`}
          </div>
        </div>
      )}
...
function LeagueDescriptionBlock({ league, canManage, onUpdateDescription, descOpen, setDescOpen, c }) {
...
function LeagueDetail({ league, session, isAdmin, joined, canSeePhones, myTeam, entryClosed, onBack, onJoin, onRecordResult, onUpdateTeamPhone, onRemoveTeam, onUpdatePhoto, onUpdateDescription, onAdvance, onGenerateFixtures, onDelete, onShare, c }) {
  ...
  return (
    <div className="pt-8">
          ...
          <div className="rounded-xl p-5 border mb-5" style={{ background: c.surface, borderColor: c.border }}>
            ...
            <div className="font-body text-sm mb-3" style={{ color: c.textDim }}>
              ...
              {isSurvivor ? ` · needs 2+ to start, cuts to ${league.survivor_target_count} over time`
                : isGroupsKnockout ? ` · needs at least ${league.groups_count * 2} to fill ${league.groups_count} groups (top ${league.group_qualifiers} from each go through)`
                : " · needs 2+ to start"}.
              {" "}Players who join automatically register their eFootball username as their club — no need to list them upfront.
            </div>
            {canManage && (
              <button disabled={league.teams.length < 2 || (isGroupsKnockout && league.teams.length < league.groups_count * 2)} onClick={() => onGenerateFixtures(league)}
                className="font-body text-sm font-semibold px-4 py-2.5 rounded-full"
                style={(league.teams.length >= 2 && !(isGroupsKnockout && league.teams.length < league.groups_count * 2)) ? { background: c.accent, color: c.accentText } : { background: c.surfaceHover, color: c.textFaint }}>
      ...
      {tab === "table" && (league.description || canManage) && (
        <LeagueDescriptionBlock league={league} canManage={canManage} onUpdateDescription={onUpdateDescription}
...
function FindYourself({ league, stageFixtures, inGroupStage, inKnockoutBracket, groupStageFixtures, canSeePhones, c }) {
  ...
  const search = () => {
    ...
    if (inKnockoutBracket) {
      ...
      const myFixture = stageFixtures.filter((f) => f.home_team_id === team.id || f.away_team_id === team.id).sort((a, b) => b.round - a.round)[0];
      setResult({ kind: "knockout", team, myFixture, isCurrentRound: myFixture?.round === maxRound, allTeams: league.teams });
      return;
    }
      ...
      {result && (result.notFound ? (
        ...
        <div className="font-body text-sm mt-3 rounded-lg px-3 py-2.5" style={{ background: c.surfaceHover }}>
          ...
          {!result.myFixture ? (
            ...
            const opp = opponentOf(result.myFixture, result.team, result.allTeams);
            if (opp.bye) return <div className="font-mono text-xs" style={{ color: c.textFaint }}>Automatic advance this round (bye).</div>;
            return (
              <div>
                <div className="font-mono text-xs" style={{ color: c.textDim }}>
                  Round {result.myFixture.round} vs <span style={{ color: c.text }}>{opp.opponent?.name}</span> ({opp.isHome ? "Home" : "Away"})
                </div>
                {result.myFixture.played ? (
                  <div className="font-mono text-xs mt-1" style={{ color: c.textDim }}>Final score: {result.myFixture.home_score} – {result.myFixture.away_score}</div>
                ) : (
                  <div className="font-mono text-xs mt-1" style={{ color: isExpired(result.myFixture) ? c.red : c.textFaint }}>
                    {isExpired(result.myFixture) ? "Expired — recorded as a loss" : `Due by ${fmtDate(result.myFixture.due_at)}`}
                  </div>
                )}
                {canSeePhones && (
                  opp.opponent?.phone
