...
export default function App() {
  ...
  // Requires the "Platform admins can view all profiles" RLS policy; a
  // non-admin calling this just gets their own row back (or none).
    ...
    <div className="min-h-screen transition-colors duration-200" style={{ background: c.bg, color: c.text, fontFamily: "'Barlow Condensed', 'Oswald', sans-serif" }}>
      ...
      <main className="max-w-3xl mx-auto px-4 pb-24">
        {view === "accounts" && isAdmin ? (
          <AccountsPanel accounts={accounts} onBack={() => setView("home")} c={c} />
...
function AccountsPanel({ accounts, onBack, c }) {
  ...
  const filtered = q
    ? accounts.filter((a) => (a.efootball_username || "").toLowerCase().includes(q) || (a.phone || "").toLowerCase().includes(q))
    : accounts;
  const flaggedCount = accounts.filter((a) => (a.phone || "").includes("(DUPLICATE-")).length;

  const exportCsv = () => {
    const rows = [["Username", "Phone", "Joined"], ...filtered.map((a) => [a.efootball_username, a.phone, a.created_at])];
  ...
  return (
    <div className="pt-8">
      ...
      <div className="relative mb-4">
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by username or phone..."
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {filtered.map((a) => <AccountRow key={a.user_id} account={a} c={c} />)}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="rounded-lg px-4 py-2.5 flex items-center gap-3" style={{ background: c.surface }}>
      ...
      {!isFlagged && digitsOnly && (
        <a href={`https://wa.me/${digitsOnly}`} target="_blank" rel="noopener noreferrer" title="Message on WhatsApp"
          className="w-7 h-7 flex items-center justify-center rounded-full shrink-0" style={{ color: c.textFaint }}>
