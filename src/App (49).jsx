...
import {
  ...
  MoreVertical, Send, CornerDownRight, Camera, Eye, ThumbsUp, ThumbsDown, Target, Snowflake, ChevronDown, History,
} from "lucide-react";
...
// The leaderboard resets every 3 months on a fixed calendar-quarter clock
// (Jan–Mar, Apr–Jun, Jul–Sep, Oct–Dec) rather than a rolling window from
// whenever someone happens to look — so everyone sees the same "season"
// boundary and it's predictable. Nothing is ever deleted or archived to
// make this work: a season is just a date filter over fixtures that were
// already played, so every past quarter stays fully browsable forever via
// the season picker in the Leaderboard.
const SEASON_QUARTER_LABEL = ["Jan – Mar", "Apr – Jun", "Jul – Sep", "Oct – Dec"];
function seasonForDate(date) {
  const d = new Date(date);
  return { year: d.getFullYear(), q: Math.floor(d.getMonth() / 3) };
}
function seasonBounds({ year, q }) {
  return { start: new Date(year, q * 3, 1, 0, 0, 0), end: new Date(year, q * 3 + 3, 1, 0, 0, 0) };
}
function seasonKey(s) { return `${s.year}-Q${s.q + 1}`; }
function seasonLabel(s) { return `Q${s.q + 1} ${s.year} · ${SEASON_QUARTER_LABEL[s.q]}`; }
function currentSeason() { return seasonForDate(new Date()); }
function nextSeason(s) { return s.q === 3 ? { year: s.year + 1, q: 0 } : { year: s.year, q: s.q + 1 }; }
function daysUntilSeasonReset() {
  const { end } = seasonBounds(currentSeason());
...
// Every quarter that has at least one played fixture in it, newest first,
// plus the current quarter even if it's still empty — this feeds the
...
function listSeasons(leagues) {
  ...
  const seasons = [];
  let s = seasonForDate(earliest);
  while (s.year < cur.year || (s.year === cur.year && s.q <= cur.q)) { seasons.push(s); s = nextSeason(s); }
...
// Picks out the top scorer and the "fewest goals" player from a set of
// leaderboard/standings rows that already expose { name, gf }. Requires at
// least one goal-scoring row to name a top scorer, and at least two
// qualifying rows before naming a separate "fewest" (otherwise it would
// just be the same person twice). Ties break alphabetically so the result
// is stable rather than order-of-insertion dependent.
function goalExtremes(rows) {
  ...
  const byLeast = [...rows].sort((a, b) => a.gf - b.gf || a.name.localeCompare(b.name));
  const top = byMost[0];
  let least = byLeast[0];
...
export default function App() {
  ...
  const recordResult = async (league, fixture, homeScore, awayScore, file = null) => {
    ...
    await postComment(league, `${homeName} ${homeScore} – ${awayScore} ${awayName}`, null, file, null, true);
    await loadLeagues();
  ...
  const approveResult = async (league, submission) => {
    ...
    if (submission.photo_path) {
      ...
      if (data?.signedUrl) {
        await postComment(
          league,
          `Photo proof for ${submission.submitted_by_username}'s approved result — ${homeName} ${submission.home_score} – ${submission.away_score} ${awayName}`,
  ...
  const rejectResult = (league, submission) => {
    requestConfirm([
      ...
      await postComment(
        league,
        `${submission.submitted_by_username}'s result was rejected — ${homeName} ${submission.home_score} – ${submission.away_score} ${awayName}`,
...
// with the fewest goals out of whatever rows were handed in — reused by
// both the platform-wide Leaderboard and each league's own Table tab, just
// scoped to a different set of rows.
function GoalExtremesBar({ top, least, c }) {
  ...
  return (
    <div className={`grid ${least ? "grid-cols-2" : "grid-cols-1"} gap-2 mb-4`}>
      ...
      {least && (
        <div className="rounded-lg px-3 py-2.5 flex items-center gap-2.5 border" style={{ background: c.surface, borderColor: c.border }}>
          <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0" style={{ background: c.surfaceHover, color: c.textDim }}><Snowflake size={13} /></div>
          <div className="min-w-0">
            <div className="font-mono text-[9px] uppercase tracking-wider" style={{ color: c.textFaint }}>Fewest goals</div>
            <div className="font-body text-xs font-semibold truncate" title={least.name}>{least.name} <span className="font-mono font-normal" style={{ color: c.textDim }}>· {least.gf}⚽</span></div>
          </div>
        </div>
      )}
    </div>
...
// 3-month quarter — the default), any past season, and an all-time view.
// Follows the same open/outside-click pattern as LeagueMenu.
function SeasonPicker({ value, seasons, onChange, c }) {
  ...
  const label = value === "all" ? "All-time" : value === seasonKey(cur) ? `This season · ${seasonLabel(cur)}` : seasonLabel(seasons.find((s) => seasonKey(s) === value) || cur);
      ...
      {open && (
        <div className="absolute left-0 mt-2 w-64 max-h-72 overflow-y-auto rounded-xl overflow-x-hidden z-20 shadow-lg" style={{ background: c.bg, border: `1px solid ${c.border}` }}>
          <button onClick={() => choose(seasonKey(cur))} className="w-full flex items-center justify-between gap-2 px-4 py-3 font-body text-sm text-left"
            ...
            This season · {seasonLabel(cur)} {value === seasonKey(cur) && <Check size={14} />}
          </button>
          {seasons.filter((s) => seasonKey(s) !== seasonKey(cur)).map((s) => (
            <button key={seasonKey(s)} onClick={() => choose(seasonKey(s))} className="w-full flex items-center justify-between gap-2 px-4 py-3 font-body text-sm text-left"
              ...
              {seasonLabel(s)} {value === seasonKey(s) && <Check size={14} />}
            </button>
          ))}
...
function Leaderboard({ leagues, session, onBack, c }) {
  ...
  const bounds = season === "all" ? null : seasonBounds(season === seasonKey(cur) ? cur : (seasons.find((s) => seasonKey(s) === season) || cur));
  ...
  return (
    <div className="pt-8">
      ...
      <div className="font-mono text-xs mb-4" style={{ color: c.textFaint }}>
        ...
        {viewingCurrent && <> · resets in {daysUntilSeasonReset()} day{daysUntilSeasonReset() === 1 ? "" : "s"}</>}
      </div>

      <div className="mb-4"><SeasonPicker value={season} seasons={seasons} onChange={setSeason} c={c} /></div>
...
function Header({ view, setView, activeLeague, theme, toggleTheme, c, onSignOut, userEmail, onEditProfile, isAdmin, onOpenAccounts, onOpenSuggestion, onOpenLeaderboard }) {
  return (
...
function LeagueSection({ title, icon, leagues, isAdmin, isMemberOf, entryClosed, myPaymentStatus, canManageLeague, onOpen, onJoin, session, onToggleLeagueReaction, c }) {
...
// Standings rows are per-club (gf, p, ...); this maps a club back to the
// member managing it so the top-scorer/fewest-goals cards can show a
// username rather than a club name, and only considers clubs that have
// actually played (a club sitting at 0 goals because it hasn't played yet
// shouldn't win "fewest goals").
