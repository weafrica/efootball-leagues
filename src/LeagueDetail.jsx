    ...
    return (
      <div className="mt-2 pt-2 border-t" style={{ borderColor: c.border }}>
        ...
        <button onClick={() => setClaiming(true)} className="flex items-center gap-1.5 font-mono text-[11px] font-semibold" style={{ color: "#B8860B" }}>
          <Zap size={11} /> {claim?.status === "rejected" ? "Claim walkover again" : "Claim walkover"}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl p-3.5 border transition-transform active:scale-[0.99]" style={{ background: c.surface, borderColor: match && !resultPending && !match.finalized_at ? c.borderStrong : c.border, boxShadow: "0 6px 18px -8px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.04)" }}>
      ...
      <LadderCupWalkoverClaimSection claim={walkoverClaim}
        onSubmitClaim={(file) => onClaimWalkover(opponent.club_id, file)} c={c} />
    </div>
  );
}
...
// and computed off entryRow.w fresh on every render, so an offer that was
// already sitting here before the pricing changed shows today's price,
// not whatever it would have shown yesterday.
function LadderCupSecondLifeOffer({ entryRow, onAccept, c }) {
  const [busy, setBusy] = useState(false);
  const deduction = winScaledFee(LADDER_CUP_RULES.SECOND_LIFE_DEDUCTION, entryRow.w);
...
function LadderCupOpponentBoard({ league, myTeam, canSeePhones, onEnsurePoolSighting, onMarkPoolContact, onInitiateMatch, onCancelMatch, onOpenResult, onRespondResult, onRespondSecondLife, onRejoin, onClaimWalkover, onBuyOpponentSlot, c }) {
  ...
  if (myStatus === "pending_second_life") {
    return <LadderCupSecondLifeOffer entryRow={myEntry._row}
      onAccept={() => onRespondSecondLife(true)} c={c} />;
  }
  ...
  const walkoverClaimWith = (opponentClubId) => {
    ...
    return against.find((cl) => cl.status === "pending_review")
      || [...against].filter((cl) => cl.status === "rejected").sort((a, b) => new Date(b.claimed_at) - new Date(a.claimed_at))[0]
      || null;
  };
  ...
  const unlockedCount = Math.min(LADDER_CUP_BASE_VISIBLE_OPPONENTS + purchasedSlots, LADDER_CUP_MAX_VISIBLE_OPPONENTS);
  const shownOpponents = visibleOpponents.slice(0, unlockedCount);
  const moreToUnlock = unlockedCount < LADDER_CUP_MAX_VISIBLE_OPPONENTS && visibleOpponents.length > unlockedCount;
  const opponentScrolls = shownOpponents.length > LADDER_CUP_OPPONENT_VISIBLE_ROWS;

  const buySlot = async () => {
    setBuyingSlot(true);
    await onBuyOpponentSlot?.();
    setBuyingSlot(false);
  };
        ...
        {claims.map((cl) => {
          ...
          return (
            <div key={cl.id} className="rounded-xl px-4 py-2.5" style={{ background: c.surface, boxShadow: "inset 0 1px 0 rgba(255,255,255,0.04)" }}>
              ...
              <div className="font-mono text-[11px]" style={{ color: c.textFaint }}>Claimed {fmtDate(cl.claimed_at)} SAST</div>
...
function LadderCupEscalatedResultRow({ match: m, home, away, reportingTeam, onApprove, onReject, onEditScore, c }) {
  ...
  const saveEdit = async () => {
    ...
    const h = editHome === "" ? 0 : parseInt(editHome, 10);
    const a = editAway === "" ? 0 : parseInt(editAway, 10);
      ...
      {editing ? (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2 w-full mt-1">
            {/* Both blank is "nothing to save"; a single blank side is a
                valid 0 (see saveEdit) so it no longer blocks Save. */}
        ...
        <div className="relative">
          ...
          {league.ladder_cup_finalized_at ? (
            ...
            <LadderCupOpponentBoard league={league} myTeam={myTeam} canSeePhones={canSeePhones} onEnsurePoolSighting={onEnsurePoolSighting} onMarkPoolContact={onMarkPoolContact} onInitiateMatch={onInitiateMatch} onCancelMatch={onCancelMatch} onOpenResult={onOpenResult} onRespondResult={onRespondResult} onRespondSecondLife={onRespondSecondLife} onRejoin={onRejoin}
              onClaimWalkover={onClaimWalkover} onBuyOpponentSlot={onBuyOpponentSlot} c={c} />
          ) : (
    ...
    <div className="pt-8">
      ...
      <div className="flex items-start justify-between mb-5">
        ...
        {joined && myPaymentStatus === "pending" && (
          notStarted ? (
            <button onClick={() => onResubmitPayment(myMembership)} className="shrink-0 flex items-center gap-2 font-extrabold text-base px-6 py-2.5 rounded-full" style={{ background: "rgba(217,164,6,0.18)", color: "#B8860B", border: "2px solid #B8860B" }}>
              <Clock size={16} /> Payment pending — complete or add
            </button>
          ) : (
      ...
      {isLadderCup ? (
        <LadderCupPendingPanel league={league} leagues={leagues} allAchievements={allAchievements} canManage={canManage} canSeePhones={canSeePhones} session={session} myTeam={myTeam} myUsername={myUsername} avatarByTeamId={avatarByTeamId}
          ...
          onPostComment={onPostComment} onDeleteComment={onDeleteComment} onEditComment={onEditComment} onEditResult={onEditResult} onCancelResult={onCancelResult} onEditLadderCupResult={onEditLadderCupResult} onToggleReaction={onToggleReaction}
          ...
          onClaimWalkover={onClaimLadderCupWalkover} onBuyOpponentSlot={onBuyLadderCupOpponentSlot}
      ...
      {tab === "table" && (
        <div>
          ...
          <CommentsSection league={league} session={session} canComment={joined || canManage}
            ...
            canEditResults={canManage}
            onPost={onPostComment} onDelete={onDeleteComment} onEdit={onEditComment} onEditResult={onEditResult} onCancelResult={onCancelResult} onToggleReaction={onToggleReaction} myUsername={myUsername} c={c} />
        </div>
      )}
        ...
        <div className="space-y-3 mb-3">
          {visibleRoots.map((cm) => (
            <CommentNode key={cm.id} comment={cm} league={league} session={session} canComment={canComment}
              onPost={onPost} onDelete={onDelete} onEdit={onEdit} onEditResult={onEditResult} onCancelResult={onCancelResult} onEditLadderCupResult={onEditLadderCupResult} canEditResults={canEditResults} onToggleReaction={onToggleReaction} c={c} depth={0} />
          ))}
        </div>
      )}
...
// ancestors.
function CommentNode({ comment, league, session, canComment, onPost, onDelete, onEdit, onEditResult, onCancelResult, onEditLadderCupResult, canEditResults = false, onToggleReaction, c, depth }) {
  ...
  return (
    <div className={comment.pending ? "opacity-60" : "comment-pop-in"}>
      <CommentRow comment={comment} league={league} session={session} canComment={canComment}
        onDelete={onDelete} onEdit={onEdit} onEditResult={onEditResult} onCancelResult={onCancelResult} onEditLadderCupResult={onEditLadderCupResult} canEditResults={canEditResults} onToggleReaction={onToggleReaction} c={c} isReply={depth > 0}
        ...
        <div className="mt-2 space-y-2 pl-3 border-l" style={{ marginLeft: indent, borderColor: c.border }}>
          {children.map((r) => (
            <CommentNode key={r.id} comment={r} league={league} session={session} canComment={canComment}
              onPost={onPost} onDelete={onDelete} onEdit={onEdit} onEditResult={onEditResult} onCancelResult={onCancelResult} onEditLadderCupResult={onEditLadderCupResult} canEditResults={canEditResults} onToggleReaction={onToggleReaction} c={c} depth={depth + 1} />
          ))}
        </div>
      )}
...
function CommentRow({ comment: cm, league, session, canComment, onDelete, onEdit, onEditResult, onCancelResult, onEditLadderCupResult, canEditResults = false, onToggleReaction, onReplyClick, c, isReply = false }) {
  ...
  const saveEdit = async () => {
    ...
    if (linkedFixture) {
      ...
      const homeNum = editHomeScore === "" ? 0 : parseInt(editHomeScore, 10);
      const awayNum = editAwayScore === "" ? 0 : parseInt(editAwayScore, 10);
      ...
      // Same blank-side-is-0 treatment as the linkedFixture branch above.
      if (editHomeScore === "" && editAwayScore === "") return;
      const homeNum = editHomeScore === "" ? 0 : parseInt(editHomeScore, 10);
      const awayNum = editAwayScore === "" ? 0 : parseInt(editAwayScore, 10);
      if (!Number.isInteger(homeNum) || !Number.isInteger(awayNum) || homeNum < 0 || awayNum < 0) return;
...
function OpponentTimezoneInfo({ league, opponentTeamId, playerLocations, myTimezone, c }) {
  const opponentUserId = ownerUserIdForTeam(league, opponentTeamId);
  const theirLocation = opponentUserId ? playerLocations?.[opponentUserId] : null;
  ...
  return (
    <div className="mt-2 pt-2 border-t flex flex-col gap-1" style={{ borderColor: c.border }}>
      {(flag || localTime) && (
...
function NextOpponentCard({ league, leagues, myTeam, canSeePhones, playerLocations, myTimezone, c }) {
  ...
  return (
    <div className="rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
      ...
      {opponent && (
        <OpponentTimezoneInfo league={league} opponentTeamId={opponent.id} playerLocations={playerLocations} myTimezone={myTimezone} c={c} />
      )}
    </div>
  );
}
    ...
    <div className="rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
      ...
      {result && (result.notFound ? (
        ...
        <div className="font-body text-sm mt-3 rounded-lg px-3 py-2.5" style={{ background: c.surfaceHover }}>
          ...
          <OpponentTimezoneInfo league={league} opponentTeamId={result.opponent.id} playerLocations={playerLocations} myTimezone={myTimezone} c={c} />
