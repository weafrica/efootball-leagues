...
import {
  ...
  FORMATS, ONE_DAY_MS, formatRand, nextMonthEndCutoffSAST, toDatetimeLocalValue, weekendWindow,
} from "./App.jsx";
...
export default function CreateLeague({ onCancel, onCreate, isAdmin, c }) {
  ...
  // until the ladder's own monthly cutoff, set below — so the generic
  ...
  // last day of the current month, 10PM SAST — the same "quick-set, still
  // overridable" pattern as Set as Weekend League above. Doesn't touch
  // entryClosesAt/startsAt; those are the generic registration-window
  // fields every format uses, separate from ladder_cup's own monthly
  // cutoff.
  const setDefaultLadderCupCutoff = () => setLadderCupCutoffAt(toDatetimeLocalValue(nextMonthEndCutoffSAST()));
  ...
  const submit = () => {
    onCreate({
      ...
      // Survival Ladder Cup is always free to play, regardless of whatever
      // leagueType was left selected from before the format was switched.
      leagueType: isAdmin && !isLadderCup ? leagueType : "fun",
    });
  };
  ...
  return (
    <div className="pt-10">
      ...
      {isAdmin && isLadderCup && (
        <div className="font-mono text-xs mb-5" style={{ color: c.textFaint }}>Survival Ladder Cup is always free to play — no entry fee, cash or Nets.</div>
      )}
      ...
      {isLadderCup && (
        <div className="font-mono text-xs mb-1.5" style={{ color: c.textFaint }}>Survival Ladder Cup has no entry-close date — clubs can join anytime until the ladder's own monthly cutoff, set below.</div>
      )}
      {isAdmin && (
