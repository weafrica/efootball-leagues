...
export default function App() {
  ...
  const toggleCommentLike = async (comment) => {
    ...
    if (mine) {
      ...
      if (error) { showToast(`Couldn't unlike: ${error.message}`); return; }
    } else {
      const { error } = await supabase.from("comment_likes").insert({ comment_id: comment.id, user_id: session.user.id });
      if (error) { showToast(`Couldn't like: ${error.message}`); return; }
    }
    await loadLeagues();
  };
...
function LeagueDetail({ league, session, isAdmin, joined, canSeePhones, myTeam, entryClosed, myPaymentStatus, onBack, onJoin, onResubmitPayment, onDownloadProof, onReviewPayment, onRecordResult, onUpdateTeamPhone, onRemoveTeam, onUpdatePhoto, onUpdateDescription, onAdvance, onGenerateFixtures, onDelete, onShare, onLeave, onPostComment, onDeleteComment, onToggleCommentLike, c }) {
  ...
  return (
    <div className="pt-8">
      ...
      <CommentsSection league={league} session={session} canComment={joined || canManage}
        onPost={onPostComment} onDelete={onDeleteComment} onToggleLike={onToggleCommentLike} c={c} />
    </div>
  );
}
...
function CommentsSection({ league, session, canComment, onPost, onDelete, onToggleLike, c }) {
  ...
  }, [league.comments, sortBy]);

  const totalCount = league.comments?.length || 0;

  const submit = async () => {
    if (!text.trim() || posting) return;
    setPosting(true);
    const ok = await onPost(league, text);
    setPosting(false);
    if (ok) setText("");
  };
  ...
  return (
    <div className="mt-8 pt-6 border-t" style={{ borderColor: c.border }}>
      ...
      {threads.length === 0 ? (
        <div className="border border-dashed rounded-xl p-6 text-center font-body text-sm mb-4" style={{ borderColor: c.borderStrong, color: c.textDim }}>
      ...
      {canComment ? (
        <div className="flex items-end gap-2">
          <textarea value={text} onChange={(e) => setText(e.target.value)} onKeyDown={onKeyDown}
            ...
            className="flex-1 font-body text-sm rounded-xl px-3 py-2.5 resize-none outline-none"
...
function CommentThread({ comment, league, session, canComment, onPost, onDelete, onToggleLike, c }) {
  ...
  const submitReply = async () => {
    ...
    setPosting(true);
    const ok = await onPost(league, replyText, comment);
    setPosting(false);
    if (ok) { setReplyText(""); setReplyOpen(false); setRepliesShown(true); }
  };
    ...
    <div>
      ...
      {repliesShown && (
        <div className="ml-9 mt-2 space-y-2 pl-3 border-l" style={{ borderColor: c.border }}>
          {replies.map((r) => (
            <CommentRow key={r.id} comment={r} league={league} session={session} canComment={canComment}
      ...
      {replyOpen && (
        <div className="ml-9 mt-2 flex items-end gap-2">
          <textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} onKeyDown={onKeyDown}
            placeholder={`Reply to ${comment.username}…`} rows={1} maxLength={1000} autoFocus
            className="flex-1 font-body text-sm rounded-xl px-3 py-2 resize-none outline-none"
            style={{ background: c.surface, color: c.text, border: `1px solid ${c.border}` }} />
...
function CommentRow({ comment: cm, league, session, canComment, onDelete, onToggleLike, onReplyClick, c, isReply = false }) {
  ...
  const likes = cm.comment_likes || [];
  const likedByMe = session && likes.some((l) => l.user_id === session.user.id);

  return (
    <div className="flex items-start gap-2.5 group">
      <div className="rounded-full flex items-center justify-center font-body font-bold shrink-0"
        style={{ background: c.green, color: c.text, width: isReply ? 22 : 28, height: isReply ? 22 : 28, fontSize: isReply ? 10 : 12 }}>
        {cm.username?.[0]?.toUpperCase() || "?"}
      </div>
      <div className="flex-1 min-w-0 rounded-xl px-3 py-2" style={{ background: c.surface }}>
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="font-mono text-[10px]" style={{ color: c.textFaint }}>{timeAgo(cm.created_at)}</span>
            {(isOwn || canComment) && (
        ...
        <div className="flex items-center gap-3 mt-1.5">
          <button onClick={() => onToggleLike(cm)} disabled={!session}
