...
export default function App() {
  ...
  const createLeague = async (input) => {
    const { name, teamNames, format, survivor, entryClosesAt, startsAt, description } = input;
    const insertPayload = {
...
function LeagueDetail({ league, session, isAdmin, joined, canSeePhones, myTeam, entryClosed, onBack, onJoin, onRecordResult, onUpdateTeamPhone, onRemoveTeam, onUpdatePhoto, onUpdateDescription, onAdvance, onGenerateFixtures, onDelete, onShare, c }) {
  ...
  return (
    <div className="pt-8">
      ...
      {tab === "table" && <StandingsPanel standings={standings} zoneFor={zoneFor} stageFixtures={stageFixtures} isSurvivor={isSurvivor} league={league} c={c} />}
