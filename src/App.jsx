...
export default function App() {
  ...
  const loadLeagues = useCallback(async () => {
    const { data, error } = await supabase
      .from("leagues")
      .select("*, teams(*), fixtures(*), members(*), comments(*)")
  ...
  const postComment = async (league, body) => {
          ...
          <>
            ...
            {view === "league" && activeLeague && (
              <LeagueDetail league={activeLeague} session={session} isAdmin={isAdmin} joined={isMemberOf(activeLeague)}
                ...
                onPostComment={postComment} onDeleteComment={deleteComment} c={c} />
            )}
          </>
        )}
      </main>
  ...
  return (
    <div className="pt-8">
      ...
      <CommentsSection league={league} session={session} canComment={joined || canManage}
        onPost={onPostComment} onDelete={onDeleteComment} c={c} />
    </div>
  );
}
...
function CommentsSection({ league, session, canComment, onPost, onDelete, c }) {
  const [text, setText] = useState("");
  const [posting, setPosting] = useState(false);
  const comments = useMemo(
    () => [...(league.comments || [])].sort((a, b) => new Date(a.created_at) - new Date(b.created_at)),
    [league.comments]
  );
  ...
  return (
    <div className="mt-8 pt-6 border-t" style={{ borderColor: c.border }}>
      <div className="font-mono text-xs uppercase tracking-[0.2em] mb-3 flex items-center gap-2" style={{ color: c.textFaint }}>
                ...
                <div className="flex-1 min-w-0 rounded-xl px-3 py-2" style={{ background: c.surface }}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="font-mono text-[10px]" style={{ color: c.textFaint }}>{fmtDate(cm.created_at)}</span>
                      {(isOwn || canComment) && (
                        <button onClick={() => onDelete(cm)} title="Delete comment"
