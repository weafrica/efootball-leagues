...
export default function App() {
  ...
  // their own call). A photo is optional here — unlike submitMatchResult's
  // mandatory proof for regular players — but if the admin attaches one it's
  // posted to the comments as scoreline + photo, same as an approved player
  // submission, so the evidence ends up visible to the whole league either way.
...
// manage the league, editable score inputs with a Save button. A joined
// non-manager instead gets a "Submit result" button that opens the photo +
// score modal — their result lands as pending until an admin approves it, or
// shows a pending/rejected tag if one's already in flight for this fixture.
...
function FixtureScoreRow({ fixture, homeTeam, awayTeam, canManage, onSave, legLabel, joined, submission, onOpenSubmitResult, c }) {
  ...
  const [photo, setPhoto] = useState(null); // admin's own optional photo proof, attached before saving
    ...
    <div className="flex items-center gap-2 py-2">
      ...
      {canManage && (
        <>
          ...
          <button onClick={() => photoInputRef.current?.click()} title={photo ? photo.name : "Attach a photo proof (optional)"}
                ...
                {canManage && (
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => { setPhotoTargetId(fixture.id); photoInputRef.current?.click(); }}
                      title={photos[fixture.id] ? photos[fixture.id].name : "Attach a photo proof (optional)"}
