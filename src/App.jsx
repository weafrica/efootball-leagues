...
import {
  ...
  MoreVertical, Send, Heart, CornerDownRight,
} from "lucide-react";
...
export default function App() {
  ...
  const loadLeagues = useCallback(async () => {
    const { data, error } = await supabase
      .from("leagues")
      .select("*, teams(*), fixtures(*), members(*), comments(*, comment_likes(*))")
  ...
  const postComment = async (league, body, parentComment = null) => {
  ...
  const toggleCommentLike = async (comment) => {
    ...
    if (mine) {
      ...
      if (error) { showToast(`Couldn't unlike: ${error.message}`); return false; }
    } else {
      const { error } = await supabase.from("comment_likes").insert({ comment_id: comment.id, user_id: session.user.id });
      if (error) { showToast(`Couldn't like: ${error.message}`); return false; }
    }
    await loadLeagues();
    return true;
  };
          ...
          <>
            ...
            {view === "league" && activeLeague && (
              <LeagueDetail league={activeLeague} session={session} isAdmin={isAdmin} joined={isMemberOf(activeLeague)}
                ...
                onPostComment={postComment} onDeleteComment={deleteComment} onToggleCommentLike={toggleCommentLike} c={c} />
            )}
          </>
        )}
      </main>
  ...
  return (
    <div className="pt-8">
      ...
      <CommentsSection league={league} session={session} canComment={joined || canManage}
        onPost={onPostComment} onDelete={onDeleteComment} onToggleLike={onToggleCommentLike} myUsername={myUsername} c={c} />
    </div>
  );
}
...
//
// Posting and liking are both optimistic: the UI reflects the action the
...
function CommentsSection({ league, session, canComment, onPost, onDelete, onToggleLike, myUsername, c }) {
  const [text, setText] = useState("");
  const [posting, setPosting] = useState(false);
  const [sortBy, setSortBy] = useState("newest"); // "newest" | "top" — top sorts by like count
  ...
  // Build a one-level thread: top-level comments (no parent) each carrying
  // their own replies, sorted oldest-first so a conversation reads top to
  // bottom like a chat log. Optimistic entries are merged in like real ones
  // so a just-posted comment or reply appears in exactly the right spot.
  const threads = useMemo(() => {
    const all = [...(league.comments || []), ...pending];
    const byParent = new Map();
    for (const cm of all) {
      if (!cm.parent_comment_id) continue;
      if (!byParent.has(cm.parent_comment_id)) byParent.set(cm.parent_comment_id, []);
      byParent.get(cm.parent_comment_id).push(cm);
    }
    const tops = all.filter((cm) => !cm.parent_comment_id).map((cm) => ({
      ...cm,
      replies: (byParent.get(cm.id) || []).sort((a, b) => new Date(a.created_at) - new Date(b.created_at)),
    }));
    if (sortBy === "top") {
      return tops.sort((a, b) => (b.comment_likes?.length || 0) - (a.comment_likes?.length || 0)
        || new Date(a.created_at) - new Date(b.created_at));
    }
    return tops.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
  }, [league.comments, pending, sortBy]);

  const totalCount = (league.comments?.length || 0) + pending.length;
  ...
  const submit = async () => {
    ...
    const ok = await onPost(league, trimmed);
    setPosting(false);
    if (!ok) {
  ...
  return (
    <div className="mt-8 pt-6 border-t" style={{ borderColor: c.border }}>
      <style>{`
        ...
        .heart-pop { animation: heartPop 0.28s cubic-bezier(0.34, 1.56, 0.64, 1); }
        .comment-textarea:focus { border-color: ${c.accent} !important; }
        @media (prefers-reduced-motion: reduce) {
          .comment-pop-in { animation: none; }
      ...
      {threads.length === 0 ? (
        ...
        <div className="space-y-3 mb-3">
          {visibleThreads.map((cm) => (
            <CommentThread key={cm.id} comment={cm} league={league} session={session} canComment={canComment}
              onPost={onPost} onDelete={onDelete} onToggleLike={onToggleLike} c={c} />
          ))}
        </div>
      )}
...
// One top-level comment plus its replies. Kept as its own component so each
// thread manages its own "reply box open?" and "replies expanded?" state
// independently of its siblings.
function CommentThread({ comment, league, session, canComment, onPost, onDelete, onToggleLike, c }) {
  ...
  useEffect(() => {
    if (replies.some((r) => r.pending)) setRepliesShown(true);
  }, [replies]);
  ...
  const submitReply = async () => {
    ...
    setReplyText("");
    setReplyOpen(false);
    const ok = await onPost(league, trimmed, comment);
    setPosting(false);
  ...
  return (
    <div className={comment.pending ? "opacity-60" : "comment-pop-in"}>
      <CommentRow comment={comment} league={league} session={session} canComment={canComment}
        onDelete={onDelete} onToggleLike={onToggleLike} c={c}
        onReplyClick={canComment ? () => setReplyOpen((v) => !v) : null} />

      {replies.length > 0 && (
        <button onClick={() => setRepliesShown((v) => !v)}
          className="ml-9 mt-1 font-mono text-[10px] uppercase tracking-wider flex items-center gap-1"
          style={{ color: c.textFaint }}>
          <CornerDownRight size={11} />
          {repliesShown ? "Hide" : "Show"} {replies.length} repl{replies.length === 1 ? "y" : "ies"}
        </button>
      )}

      {repliesShown && (
        <div className="ml-9 mt-2 space-y-2 pl-3 border-l" style={{ borderColor: c.border }}>
          {replies.map((r) => (
            <div key={r.id} className={r.pending ? "opacity-60" : "comment-pop-in"}>
              <CommentRow comment={r} league={league} session={session} canComment={canComment}
                onDelete={onDelete} onToggleLike={onToggleLike} c={c} isReply />
            </div>
          ))}
        </div>
      )}
...
// league creator), timestamp, delete, body, and a like button with count.
// The like button is optimistic — it flips instantly on click and only
// reconciles with the real comment_likes list once that reload lands.
function CommentRow({ comment: cm, league, session, canComment, onDelete, onToggleLike, onReplyClick, c, isReply = false }) {
  const isOwn = session && cm.user_id === session.user.id;
  const isManager = cm.user_id === league.created_by;
  const realLikes = cm.comment_likes || [];
  const realLikedByMe = !!(session && realLikes.some((l) => l.user_id === session.user.id));

  const [pendingLike, setPendingLike] = useState(null); // null = no optimistic override in flight
  const [popKey, setPopKey] = useState(0);
  useEffect(() => {
    if (pendingLike !== null && pendingLike === realLikedByMe) setPendingLike(null);
  }, [realLikedByMe]); // eslint-disable-line react-hooks/exhaustive-deps

  const likedByMe = pendingLike !== null ? pendingLike : realLikedByMe;
  const likeCount = realLikes.length + (pendingLike !== null && pendingLike !== realLikedByMe ? (pendingLike ? 1 : -1) : 0);
  ...
  const handleLike = async () => {
    ...
    const ok = await onToggleLike(cm);
    if (!ok) setPendingLike(null);
  };
        ...
        {!cm.pending && (
          <div className="flex items-center gap-3 mt-1.5">
            <button onClick={handleLike} disabled={!session}
              ...
              style={{ color: likedByMe ? c.red || "#e5484d" : c.textFaint }}>
              <Heart key={popKey} size={12} fill={likedByMe ? "currentColor" : "none"} className={popKey > 0 ? "heart-pop" : ""} />
