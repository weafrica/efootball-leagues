...
const FORMATS = [
  ...
  // available: false until Step 7 (CreateLeague wiring) lands — CreateLeague.jsx
  // already renders every FORMATS entry as a selectable button gated by
  // `available` ("Coming soon" badge + disabled otherwise), so leaving this
  // true now would let someone create a ladder_cup league with none of its
  // config (cutoff date, etc.) and no ladder-cup-aware fixture generation,
  // silently falling through to the default round-robin fixture path instead.
  // Flip to true in Step 7 once creation actually builds ladder_cup_entries.
  { id: "ladder_cup", label: "Survival Ladder Cup", kind: "ladder_cup", desc: "Ranked ladder, one elimination life each. Most points by the Sunday cutoff wins.", available: false },
];
