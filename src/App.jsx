  ...
  const joinLeague = async (leagueId) => {
    ...
    if (match) {
      ...
      const { data: newTeam, error: teamErr } = await supabase.from("teams")
        ...
        .select().single();
      if (teamErr) { showToast(`Couldn't register your club: ${teamErr.message}`); return; }
      match = newTeam;
    }
...
function CreateLeague({ onCancel, onCreate, c }) {
  ...
  const canCreate = name.trim().length > 0 && (teamNames.length === 0 || teamNames.length >= 2) && survivorValid && entryClosesAt && startsAt;
...
function LeagueDetail({ league, session, isAdmin, joined, canSeePhones, myTeam, entryClosed, onBack, onJoin, onRecordResult, onUpdateTeamPhone, onAdvance, onGenerateFixtures, onDelete, onShare, c }) {
        ...
        <div className="font-body text-sm mt-3 rounded-lg px-3 py-2.5" style={{ background: c.surfaceHover }}>
          ...
          {canManage && (
            <div className="flex items-center gap-2 mt-3 pt-3 border-t" style={{ borderColor: c.border }}>
              <span className="font-mono text-xs" style={{ color: c.textFaint }}>Enter result{result.expired ? " (expired)" : ""}:</span>
