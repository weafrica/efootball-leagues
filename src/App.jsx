...
import {
  ...
  ArrowLeft, Settings2, Moon, Sun, LogOut, Lock, Crown, Layers,
} from "lucide-react";
...
function toFixtureRows(leagueId, rounds, stage, roundOffset = 0) {
  const rows = [];
  rounds.forEach((round, ri) => {
    round.forEach(({ home, away }) => {
      const bye = away === null;
      rows.push({
        league_id: leagueId, round: ri + 1 + roundOffset, stage,
...
export default function App() {
  ...
  const createLeague = async (input) => {
    const { name, teamNames, format, survivor } = input;
    const insertPayload = { name, created_by: session.user.id, format };
    if (format === "survivor") {
    ...
    let fixtureRows = [];
    let startsInFinal = false;

    if (format === "single_round_robin") {
      fixtureRows = toFixtureRows(league.id, roundRobin(teamIds), 1);
    } else if (format === "double_round_robin") {
      fixtureRows = toFixtureRows(league.id, doubleRoundRobin(teamIds), 1);
    } else if (format === "knockout") {
      fixtureRows = toFixtureRows(league.id, [knockoutRound1(teamIds)], 1);
    } else if (format === "survivor") {
      if (teamIds.length <= survivor.targetCount) {
        fixtureRows = toFixtureRows(league.id, finalStageSchedule(teamIds, survivor.finalFormat), 1);
        startsInFinal = true;
      } else {
        fixtureRows = toFixtureRows(league.id, stageSchedule(teamIds, survivor.matchesPerStage), 1);
      }
    }
    ...
    if (startsInFinal) {
      await supabase.from("leagues").update({ final_stage_started: true }).eq("id", league.id);
    }
  ...
  const advanceKnockout = async (league) => {
    ...
    const fixtureRows = toFixtureRows(league.id, [knockoutRound1(winners)], 1, maxRound);
    const ok = await insertChunked("fixtures", fixtureRows, showToast);
    if (!ok) return;
    await loadLeagues();
  ...
  const advanceSurvivor = async (league) => {
    ...
    const fixtureRows = toFixtureRows(league.id, rounds, nextStage);
    const ok = await insertChunked("fixtures", fixtureRows, showToast);
    if (!ok) return;
...
function CreateLeague({ onCancel, onCreate, c }) {
  ...
  const canCreate = name.trim().length > 0 && teamNames.length >= 2 && survivorValid;
    ...
    <div className="pt-10">
      ...
      {format === "survivor" && (
        <div className="rounded-lg p-4 border mb-5" style={{ background: c.surface, borderColor: c.border }}>
          ...
          <p className="font-body text-xs mt-3" style={{ color: c.textFaint }}>
            Each stage, every remaining club plays {matchesPerStage || "?"} matches against different opponents. The bottom {eliminationPercent || "?"}% by points is cut. This repeats until {targetCount || "?"} clubs remain, who then finish the league with a {finalFormat === "double_round_robin" ? "double" : "single"} round robin.
          </p>
        </div>
      )}

      <label className="block font-mono text-xs uppercase tracking-wider mb-2" style={{ color: c.textDim }}>Clubs <span style={{ color: c.textFaint }}>(one per line, min. 2 — use each player's eFootball username)</span></label>
      <textarea value={teamsText} onChange={(e) => setTeamsText(e.target.value)} rows={8} placeholder={"Ndosi_123\nAsonele2k\nAshozi_10\nTheAnimal5"} className="w-full border rounded-lg px-4 py-2.5 font-body outline-none resize-none" style={inputStyle} />
...
function LeagueDetail({ league, session, joined, canSeePhones, myTeam, onBack, onJoin, onRecordResult, onUpdateTeamPhone, onAdvance, c }) {
    ...
    <div className="pt-8">
      ...
      {isSurvivor && !survivorComplete && (
        <div className="rounded-xl p-4 mb-5 border" style={{ background: c.surface, borderColor: c.border }}>
          ...
          {isCreator && !league.final_stage_started && (
      ...
      {isCreator && isKnockout && !knockoutChampion && (
      ...
      {tab === "fixtures" && (
        <div className="space-y-6">
          {joined && (
            <OpponentFinder teams={league.teams} fixtures={stageFixtures} totalRounds={totalRounds} isCreator={isCreator}
...
function OpponentFinder({ teams, fixtures, totalRounds, isCreator, canSeePhones, onRecordResult, c }) {
        ...
        <div className="font-body text-sm mt-3 rounded-lg px-3 py-2.5" style={{ background: c.surfaceHover }}>
          ...
          {isCreator && (
            <div className="flex items-center gap-2 mt-3 pt-3 border-t" style={{ borderColor: c.border }}>
              ...
              <button onClick={() => onRecordResult(result.fixture, h, a)} className="font-body text-xs font-semibold px-3 py-1.5 rounded-full ml-auto" style={{ background: c.accent, color: c.accentText }}>Save</button>
            </div>
          )}
        </div>
      ))}
