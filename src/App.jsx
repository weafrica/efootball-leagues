...
// initial render ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â bundling them in eagerly meant every single visitor
...
// challenge reference content) that only a fraction of visitors ever open ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// user tapping into a specific league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never on the guest/login page ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so
...
// itself needs the pure engine ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â isValidMatchLength backstops the same
// 6ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“15 range the DB CHECK constraint enforces. Home/away assignment now
...
// engine's assignHomeTeam isn't imported here anymore ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it's still used
...
// derived as roundPeriodMs(league) * 2 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but that silently gave ties a
...
// column value ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fall back to the original fixed 48-hour (2-day) gap so their
...
// nobody could see ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this skips the actual fetch while hidden, and catches
...
// existing loader, e.g. loadLadder) whenever a row changes ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â rather than
...
// a slow safety net ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a dropped realtime connection (which does happen on
...
// The "Shop now" banner opens the in-app WeAfrica Shop (see Shop.jsx) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// Promo badge on the shop banner ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â flip SHOP_PROMO_ACTIVE to true whenever
...
// Every cash league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â however it ends ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reserves a flat 5% of the pool for
...
// how much that member personally put in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same "the more you put in, the
// bigger your prize" rule ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â and any shortfall from underpayment gets
...
const KNOCKOUT_PRIZE_SPLIT = [0.75, 0.20]; // champion, runner-up ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â sums to 0.95, leaving the organizer's 0.05
const ROUND_ROBIN_PRIZE_SPLIT = [0.55, 0.25, 0.15]; // gold, silver, bronze ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â sums to 0.95, leaving the organizer's 0.05

function isKnockoutFormat(league) {
  return league.format === "knockout" || league.format === "groups_knockout";
}

// Which prize-split array applies to this league's format ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see the module
...
// The organizer's flat 5% cut of any cash league's pool ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â 0 if the league
...
// Optional accent-color choices layered on top of THEMES.dark/light above ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// The Ladder gets its own look ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â black, gold and red, matching the "Ladder
// Battles / No Mercy" badge ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â instead of following the app's normal
...
// Ladder's components already thread it through ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â buttons, modals, rows ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
export const FORMATS = [
  ...
  { id: "double_round_robin", label: "Double Round Robin", kind: "round_robin", desc: "Home and away ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every club plays every other club twice.", available: true },
  ...
  // one-active-fun-league-per-kind join lock the other formats share ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // board (Step 9) still aren't wired up ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â LeagueDetail shows a holding
...
// True if the signed-in user has a club actively playing in `l` right now ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// free to join. Only fun leagues are restricted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â cash leagues never lock
// each other out.
...
// A round that pairs down to exactly one real matchup IS the final ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it's
...
// due date ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â defaults to roundNumber so existing callers that pass a fixed
...
// advanceKnockout) need to pass dueOffset: 1 explicitly ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â otherwise the
...
export function knockoutRoundFixtures(leagueId, teamIds, stage, roundNumber, dueBase, legs, periodMs = TWO_DAYS_MS, dueOffset = roundNumber, isWeekend = false) {
  ...
  // Two-legged ties share ONE deadline covering both matches ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â double the
  // normal single-round window (e.g. 4 days instead of 2) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â instead of each
  ...
  // ever adjusted after creation (a dispute extension, a manual edit) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
function generateOpeningFixtures(league, teamIds, dueBase) {
  ...
  // pause (see addPausableDuration) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every fixture-row builder below is
  ...
  if (format === "groups_knockout") {
    // Groups are sized to the admin's chosen "players per group" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the number of
...
// tie ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no other simultaneous tie, and not a bye ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â because whoever wins
...
// Same check, scoped down to whichever tie a single fixture belongs to ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// a penalty entry for that side ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â unlike a goal, "no penalties recorded
...
// due_at is advisory only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â still shown to nudge players on when they're
// expected to play ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but it no longer blocks submission or auto-scores a
...
// unplayed. Both teams are eliminated the moment that's true ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â regardless
...
// counts once BOTH legs are missed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not just one, since the other leg may
// still genuinely decide it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â while a plain single-match fixture (round
...
// stage, and format ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not only knockout, and not only once a round is
...
function findNoShowTeamIds(league) {
  ...
  // points-based ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a group stage, a Survivor stage, plain round robin ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // -4 goal difference ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see computeStandings) but doesn't, on its own,
  ...
  // knockout round 1 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the exact case this guards against now.
...
// Earliest not-yet-played, fully-paired fixture across the whole league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// still "due" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it stays played:false forever in the DB, so it has to be
...
// True if `league` counts as a weekend league right now ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same definition
...
// starts_at that falls in the current FriÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Sun window. Deliberately doesn't
// reuse the spotlight's extra "still has matches due this weekend" reach ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// queue ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â 10 minutes instead for a weekend league (see isWeekendLeague),
...
// 30-minute window as league fixtures above ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â both tables store the report
...
// straight to the admin queue ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â two honest mistakes is a reasonable benefit
...
// attempt (like `result_submissions`) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so there's no submissions list to
...
// fielded a club in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â used for the "Up next" strip at the top of Home.
...
// always moves the bar ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â wins (25) and draws (10) just move it faster. This
...
// XP required to climb out of `level` into `level + 1` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a slowly rising
...
// league's members list ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the same lookup myTeam() does for the signed-in
...
export function computeMyProgress(leagues, myTeam) {
  ...
  // Current win streak ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â consecutive wins counting back from the most
  ...
  // Career-best win streak ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the longest run of consecutive wins anywhere
  ...
  // Career-best unbeaten run (wins + draws) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a looser cousin of the win
  ...
  // Wins with a shutout at the back, and the biggest winning margin ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â feed
...
// Purely cosmetic rank names for the level badge ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a light "there's more to
...
// The next title tier up from the given level ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â what the "next: X" hint in
...
// A distinct color per title tier ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fixed hex, independent of the active
// theme, the same way the ladder's top-3 medal colors work ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so climbing
...
function tierColorFor(level) {
  if (level >= 21) return "#FFD700"; // Legend ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â gold
  if (level >= 16) return "#F97316"; // Elite ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â orange
  if (level >= 11) return "#A855F7"; // Ace ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â purple
  if (level >= 6) return "#3B82F6"; // Veteran ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â blue
  if (level >= 3) return "#22C55E"; // Contender ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â green
  return "#9CA3AF"; // Rookie ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â neutral gray
}

// The tap target for the Home player card's level/XP row ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â spells out the
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(0,0,0,0.55)" }} onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl p-6 border" style={{ background: c.bg, borderColor: c.borderStrong }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="flex items-center gap-1.5 font-mono text-xs font-bold uppercase tracking-wider" style={{ color: tier }}>
              <Star size={13} /> Level {progress.level} Ãƒâ€šÃ‚Â· {progress.levelTitle}
            </div>
            <div className="font-body text-xs mt-1" style={{ color: c.textDim }}>
              {xpToGo} XP to Level {progress.level + 1}{next && next !== progress.levelTitle ? ` Ãƒâ€šÃ‚Â· next: ${next}` : ""}
            </div>
          </div>
        ...
        <div className="font-body text-[11px] leading-relaxed" style={{ color: c.textFaint }}>
          {progress.played} career {progress.played === 1 ? "match" : "matches"} Ãƒâ€šÃ‚Â·{" "}
          Wins are worth 25 XP, draws 10 XP, losses 5 XP ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every match you play moves the bar.
        </div>
      </div>
    </div>
  );
...
// Milestone badges layered on top of the level/XP system ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a second, more
...
export const ACHIEVEMENTS_DEF = [
  ...
  // Gates the Wall of Fame (see computeWallOfFame) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â only members holding
  // this badge show up there. "Won" means owning (via `members`) the team
  // that ends up champion of a completed league, any format ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see
  // computeMyLeagueWins.
...
// Fixed display order + label for each achievement category ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â used to group
...
// Visual weight per rarity tier ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â how thick the ring is. Gold and platinum
...
// Trophy-score weighting per tier for the Wall of Fame ranking ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a platinum
...
// every member) into one ranked row per member ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â count earned, a weighted
...
// profile (memberAvatars only lists other members ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the signed-in player's
...
// specific league(s) each winner actually won, and when ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the badge alone
...
// something worth chasing next ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never a random pick from the middle.
...
// A single badge tile ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a filled, colored ring around the icon once earned
...
// Homepage teaser ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a horizontally-scrolling row of badges (earned first,
...
// The full achievements list ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every badge, earned and locked, grouped by
        ...
        {groups.map((group, i) => (
          <div key={group.id} className={i > 0 ? "mt-6" : ""}>
            ...
            <div className="grid grid-cols-3 gap-x-2 gap-y-4">
              {group.items.map((a) => (
                <div key={a.id} className="flex flex-col items-center gap-1" title={`${a.desc} Ãƒâ€šÃ‚Â· ${a.tier}`}>
...
// Compact homepage preview of the platform-wide Wall of Fame ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â top 3 by
            ...
            <div className="flex flex-col leading-tight">
              ...
              {row.titles.length > 0 ? (
                <span className="font-mono text-[10px] truncate max-w-[130px]" style={{ color: "#FFD700" }}>
                  ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Ëœ {row.titles[0].leagueName}
                </span>
              ) : (
...
// The full Wall of Fame ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every member who's actually won a league (holds
// the league_champion badge ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see computeWallOfFame), ranked by trophy
      ...
      <div className="w-full max-w-sm max-h-[85vh] overflow-y-auto rounded-2xl p-6 border" style={{ background: c.bg, borderColor: c.borderStrong }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <div className="font-body text-xs mt-1" style={{ color: c.textDim }}>Ranked by trophy score ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â rarer badges count for more</div>
          </div>
          <button aria-label="Close" onClick={onClose} style={{ color: c.textFaint }}><X size={18} /></button>
        </div>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0 leading-tight">
                    ...
                    <div className="font-mono text-[10px]" style={{ color: c.textFaint }}>
                      {hasTitles ? `${row.titles.length} title${row.titles.length > 1 ? "s" : ""} Ãƒâ€šÃ‚Â· ${row.count} badges` : `${row.count} badges`}
                    </div>
                  </div>
                  {row.bestBadge && (
                  ...
                  <div className="flex flex-col gap-1 mt-2 pl-8">
                    {row.titles.map((t) => (
                      <div key={t.leagueId} className="flex items-center gap-1.5 font-body text-xs" style={{ color: c.text }}>
                        ...
                        <span className="shrink-0" style={{ color: c.textFaint }}>Ãƒâ€šÃ‚Â· {formatTitleDate(t.wonAt)}</span>
                      </div>
                    ))}
                  </div>
                )}
...
// deadline ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â and, per the no-show rule, both sides also concede 4 goals
...
// scoreline ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the fixture itself stays unplayed/scoreless in the database;
...
export function computeStandings(teams, fixtures, league) {
  ...
  // (0 pts, 0 gd ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nothing on the board either way) would otherwise tie
...
// How many of `leagues` the given user has actually won ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â checked the same
...
// "Jul 2026" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â deliberately coarser than a full date. A trophy cabinet
// remembers the season, not the exact afternoon.
function formatTitleDate(iso) {
  return new Date(iso).toLocaleDateString(undefined, { month: "short", year: "numeric" });
}

// Same champion-determination logic as computeMyLeagueWins just above ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
// same three branches (knockout bracket / survivor final stage / round-
// robin standings), same "team owner via members" attribution ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but run
...
// Points-table standings don't reflect a bracket properly ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â two teams that
...
// goal difference across their exit-round leg(s) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the same aggregate rule
...
// match ever played on the platform ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not a fixed calendar quarter ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so
...
// The date of the first match anyone ever played, across every league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// hasn't been claimed by a signed-up member yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â e.g. a name from a
// league's pre-listed open-registration team sheet that nobody has joined
// under ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â has no user to attribute its results to, so it's kept as its own
...
// fixtures/teams directly) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â which is exactly why a club leading its
...
// for every member who actually won a place (top 3 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â gold/silver/bronze ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
// for round-robin-ending leagues, champion + runner-up for
// knockout/groups_knockout ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see cashPrizePercentages ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â among
...
// their own direct prize ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see the module comment near ORGANIZER_SHARE for
...
// live projection or the final result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see memberBalance and the
// "started/complete" lifecycle below. Does NOT include the organizer's
// cut ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â that's a flat fee, not a member prize, computed separately by
// organizerFee().
...
// members always show R0.00 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â they haven't put anything in yet.
...
// The one path we don't control ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the security-definer approve_result_submission
// SQL function posting its own "under the submitter's identity" comment ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â predates
// that column, so this also recognises the scoreline shape it writes
// ("Home 2 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ 1 Away") as a fallback, keeping older/DB-side result posts grouped
...
export function isResultComment(body, isResultFlag) {
  ...
  if (body.includes("approved result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â") || body.includes("result was rejected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â")) return true;
  return /^.+\s\d+\s*ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“\s*\d+\s.+$/.test(body.trim());
}

// Splits a league's flat comment list into two flat lists ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â "results" and
// "regular" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â by walking each comment up to its root and classifying by the
...
// *other* side of that fixture ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the one who should be confirming or
...
// device timezone ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so every player and admin sees the exact same time for
...
// Sunday 23:59:59 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â "this weekend" if today already falls in that window,
...
// wall-clock window too ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not whatever timezone the visitor's device happens
// to be in. South Africa doesn't observe DST, so SAST is a fixed UTC+2.
const SAST_OFFSET_MS = 2 * 60 * 60 * 1000;

// True from 9pm through 8:59am SAST ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the overnight stretch the Weekend
...
// `override` ("paused" | "live" | null) is an admin's manual call ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see
// weekendOverride in App() ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â and always wins over the clock when set, so an
...
// weekend leagues the 9pm-9am SAST pause doesn't count toward that time ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
function addPausableDuration(startDate, durationMs, isWeekend) {
  ...
  while (remaining > 0 && guard < 10000) {
    ...
    if (isWeekendPauseHour(cursor)) {
      // Currently in the paused window ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â jump straight to 9am, none of
      // this stretch counts against `remaining`.
      cursor = nextSastHourBoundary(cursor, 9);
      continue;
    }
    // Active window ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â consume time up to the next 9pm pause, or all of
...
// (Africa/Johannesburg, UTC+2 fixed, no DST ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same rule as fmtDate/
...
// <input type="datetime-local"> expects, in the browser's local time ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the
...
// Short relative timestamp for comments/replies ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â falls back to the full
...
// Deterministic avatar background per username ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same person always gets the
...
// hits 0 nothing resolves it automatically ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it just becomes visible in the
...
// WhatsApp's own brand green ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â kept constant across both themes so the button
// reads as "WhatsApp" at a glance rather than blending into the app's palette.
const WHATSAPP_GREEN = "#25D366";

// The one support line for the whole site ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â shown as a floating button on
...
// "I messaged them recently" flag ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not tied to any fixture due date.
const WA_REMINDER_WINDOW_MS = 24 * 60 * 60 * 1000;

// Builds a wa.me deep link with an optional prefilled message. wa.me opens
// whichever WhatsApp variant ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â regular or Business ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â is installed as the
...
// Appended only to matchday-1 "let's arrange the match" texts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a brand new
...
export function firstMatchdayNote(round) {
  if (round !== 1) return "";
  return `\nÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Â  Also, jump on ${SITE_URL} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you'll find your opponent right at the top of the homepage`;
}
...
// iconOnly, renders as a plain round icon button and drops the text label ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// to calls a user already created inside the app) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so this opens the
...
// Site-wide quick-contact entry point ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a floating round WhatsApp button
...
// opposite corner ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â keeps the Terms & Conditions one tap away from every
...
// The "referee" notification mascot ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â drop-in replacement for the old
...
// (see the queueing logic in App()) just for a bit of visual variety ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
function RefereeNotification({ data, c }) {
  ...
  // Same singleton used by the rules player and comment rows ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â tapping this
  ...
  // the `pos` classes passed in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â gives the bubble a "targeting frame" look
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-0 sm:px-4" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-5 max-h-[92vh] overflow-y-auto" style={{ background: c.bg, border: `1px solid ${c.border}` }}>
        ...
        <div className="font-body text-[11px] mb-4" style={{ color: c.textFaint }}>
          The more you put in, the bigger your prize ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â {formatRand(ENTRY_FEE_MAX)} is the max contribution (100% share). Your prize for a place is scaled by your entry as a fraction of {formatRand(ENTRY_FEE_MAX)}.
        </div>
        ...
        <label className="block font-mono text-xs uppercase tracking-wider mb-2" style={{ color: c.textDim }}>
          Entry fee <span style={{ color: c.textFaint }}>({formatRand(ENTRY_FEE_MIN)}ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“{formatRand(ENTRY_FEE_MAX)})</span>
        </label>
        ...
        <button disabled={!file || saving} onClick={submit} className="w-full flex items-center justify-center gap-2 font-body font-semibold px-5 py-3 rounded-full"
          ...
          {saving ? "SubmittingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : `Submit ${formatRand(clampFee(fee))} for approval`}
        </button>
      </div>
    </div>
  );
...
function SubmitResultModal({ league, fixture, homeTeam, awayTeam, existing, onCancel, onSubmit, c }) {
  ...
  // The final is always a single decisive match ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â if it's tied, penalties
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-5 max-h-[92vh] overflow-y-auto" style={{ background: c.bg, border: `1px solid ${c.border}` }}>
        ...
        {existing?.review_note && (
          <div className="rounded-lg p-3 mb-4 font-body text-xs" style={{ background: c.redSoft, color: c.red }}>
            <div className="font-mono text-[10px] uppercase tracking-wider mb-1">Rejected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â admin's note</div>
            {existing.review_note}
          </div>
        )}
        ...
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="self-end pb-2" style={{ color: c.textFaint }}>ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“</span>
          <div className="flex-1 min-w-0">
        ...
        {needsPens && (
          <div className="mb-5">
            <div className="font-mono text-xs mb-2" style={{ color: c.red }}>This is the final ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â level after regulation goes to penalties.</div>
            <div className="flex items-center gap-2">
              <div className="flex-1 min-w-0">
            ...
            {ph !== "" && pa !== "" && Number(ph) === Number(pa) && (
              <div className="font-mono text-[10px] mt-1" style={{ color: c.red }}>Penalties can't be level too ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â someone has to win.</div>
            )}
          </div>
        )}
        ...
        <div className="font-mono text-[11px] mb-5" style={{ color: c.textFaint }}>
          The admin reviews this before it counts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â once approved it's posted to the comments under your name automatically.
        </div>
        ...
        <button disabled={!file || saving || !pensReady} onClick={submit} className="w-full flex items-center justify-center gap-2 font-body font-semibold px-5 py-3 rounded-full"
          ...
          {saving ? "SubmittingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "Submit for admin approval"}
        </button>
      </div>
    </div>
  );
...
// proof here (unlike league results) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it's a friendly 1v1, and the other
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-0 sm:px-4" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-5 max-h-[92vh] overflow-y-auto" style={{ background: c.bg, border: `1px solid ${c.border}` }}>
        ...
        <button disabled={!file || saving} onClick={submit} className="w-full flex items-center justify-center gap-2 font-body font-semibold px-5 py-3 rounded-full"
          style={file && !saving ? { background: c.accent, color: c.accentText } : { background: c.surface, color: c.textFaint }}>
          {saving ? "LoggingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "Log result"}
        </button>
      </div>
    </div>
  );
...
// pipeline (submitLadderCupMatchResult below) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it just collects the
...
// manual choice ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â resolveMatchWinner derives it from whichever scoreline
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-0 sm:px-4" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-5 max-h-[92vh] overflow-y-auto" style={{ background: c.bg, border: `1px solid ${c.border}` }}>
        ...
        {regulationLevel && (
          <div className="mb-4">
            <div className="font-mono text-xs mb-2" style={{ color: c.textDim }}>Level after regulation ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â extra time score</div>
            <div className="flex items-center gap-2">
              <div className="flex-1 min-w-0">
        ...
        {needsPens && (
          <div className="mb-4">
            <div className="font-mono text-xs mb-2" style={{ color: c.red }}>Still level after extra time ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â penalties</div>
            <div className="flex items-center gap-2">
              <div className="flex-1 min-w-0">
                <div className="font-body text-xs truncate mb-1" style={{ color: c.textDim }}>{homeTeam?.name || "Home"} (pens)</div>
                <input type="number" min={0} value={ph} onChange={(e) => setPh(e.target.value === "" ? "" : Number(e.target.value))}
        ...
        <div className="font-mono text-[11px] mb-5" style={{ color: c.textFaint }}>
          This posts straight to the ladder ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â points, streaks, and elimination update immediately, no admin review.
        </div>

        <button disabled={!file || saving || !pensReady} onClick={submit} className="w-full flex items-center justify-center gap-2 font-body font-semibold px-5 py-3 rounded-full"
          style={file && !saving && pensReady ? { background: c.accent, color: c.accentText } : { background: c.surface, color: c.textFaint }}>
          {saving ? "SavingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "Log result"}
        </button>
      </div>
    </div>
  );
...
// utterance at a time ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so "which comment is currently being read aloud"
...
if (typeof window !== "undefined" && window.speechSynthesis) {
  ...
  // doesn't clobber ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â or get clobbered by ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â RulesModal's own voice loading.
...
// Only one audio source ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a "read aloud" utterance or a voice-note clip ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
export const commentSpeech = {
  ...
  speak(id, text) {
    ...
    // in RulesModal ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â harmless no-op on browsers that don't have it.
...
// own instance of this, same as they each get their own text/photo state ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
export function useVoiceRecorder() {
  ...
  // Puts a previously-recorded clip back into the "recorded" preview state ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// Mic button for a comment composer: idle ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ tap to start recording ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ tap
...
export function VoiceNotePlayer({ url, duration, c, compact = false }) {
  ...
  const toggle = () => {
    if (!audioRef.current) {
      // Same fix as photos/avatars ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â old voice notes recorded before the
...
// Small pill button that opens a RulesModal ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â dropped in wherever a player
...
export default function App() {
  ...
  // 9pmÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“9am SAST schedule as usual; "paused" / "live" forces that state
  ...
  // every weekend league at once ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see APP-SETTINGS-MIGRATION.md.
  ...
  // always starts from these defaults ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but the browser itself preserves
  ...
  const [refereeQueue, setRefereeQueue] = useState([]); // [{ id, msg }] ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â messages waiting to be shown
  const [activeReferee, setActiveReferee] = useState(null); // { id, msg, variant, phase: "in" | "hold" | "out" } ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â currently on screen
  const [theme, setTheme] = useState(() => {
    try { return localStorage.getItem(THEME_KEY) || "dark"; } catch (e) { return "dark"; }
  });
  // Accent color: originally a guest-only preference (picked on PublicHome,
  // before signing in) that never made it past that screen ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a guest who
  ...
  const setAccent = (key) => {
    setAccentKey(key);
    try { localStorage.setItem(ACCENT_KEY, key); } catch (e) { /* ignore ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â storage unavailable */ }
  };
  const [handledDeepLink, setHandledDeepLink] = useState(false);
  const [paymentModal, setPaymentModal] = useState(null); // { league, member } ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â member set only when resubmitting
  const [resultModal, setResultModal] = useState(null); // { league, fixture, homeTeam, awayTeam, existing } ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â existing set only when resubmitting a rejected result
  // Set when a player taps an "Up next" card on Home wanting to log that
  // specific fixture's result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â activeLeagueId flips first and the league's
  ...
  const [challengeResultModal, setChallengeResultModal] = useState(null); // { kind: "challenge" | "open", challenge } ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â logging a score for an accepted challenge
  const [ladderCupResultModal, setLadderCupResultModal] = useState(null); // { league, match } ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â step 10, logging a Ladder Cup match result
  ...
  const [allAchievements, setAllAchievements] = useState(null); // every earned badge, every member ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â feeds the Wall of Fame
  ...
  const [openChallenges, setOpenChallenges] = useState(null); // broadcast "random challenge" pool ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â open to whoever accepts first
  ...
  const [ladder, setLadder] = useState(null); // the whole permanent ladder, ordered by rank_position ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never resets
  ...
  const [shopDeepLinkProductId, setShopDeepLinkProductId] = useState(null); // from a shared /shop/<id> link ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â works signed in or as a guest
  ...
  // <html>/<body> behind it never did ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â on mobile, an edge swipe triggers
  ...
  // Same call signature as the old showToast(msg) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every existing call
  ...
  // showing ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so firing several actions in quick succession (e.g.
  ...
  // Whichever comment/notification is currently being read aloud ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â shared
  ...
  useEffect(() => {
    ...
    if (activeReferee.phase === "hold") {
      // Someone tapped the speaker on this notification ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â hold off the
  ...
  // "out" animation rather than waiting on ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â or restarting ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the hold
  // timer above.
  ...
  // single window.confirm() ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â makes an accidental tap or misclick far less likely to
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, s) => {
      setSession(s);
      // Step 1 of activity tracking ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â just sign-in/sign-out for now, more
  ...
  const toggleTheme = () => {
    ...
    setTheme(next);
    try { localStorage.setItem(THEME_KEY, next); } catch (e) { /* ignore ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â storage unavailable */ }
  };

  // `stay` is set right before the redirect fires, not baked into the
  // client at load time ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the Google sign-in flow leaves the page and
  ...
  // screens) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â unlike the guest bundle, `leagues`/`teams`/`fixtures`/
  ...
  // `*` here ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â narrowing them would be high risk for very little payoff.
  //
  ...
  // over a league's lifetime, and they're consumed in far fewer places ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // by any renderer) purely so toggleCommentReaction/deleteComment ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â which
  // only ever receive a bare comment, not its parent league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â can resolve
  ...
  // Every mutation used to follow itself with a full loadLeagues() ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a
  ...
  // shape, and merge the result(s) into local state by id ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so the "server
  ...
  const refreshLeague = useCallback(async (leagueId) => {
    ...
    if (error) { showToast("Couldn't refresh the league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try reloading."); return; }
  ...
  const refreshLeagues = useCallback(async (leagueIds) => {
    ...
    if (error) { showToast("Couldn't refresh leagues ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try reloading."); return; }
    mergeLeaguesById(data || []);
  }, [showToast, mergeLeaguesById]);



  // Public setting (no auth required to read ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â guests need it too, see
  ...
  const loadWeekendOverride = useCallback(async () => {
    ...
    if (error) return; // table may not exist yet if the migration hasn't been run ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fail quiet, spotlight just falls back to the auto schedule
  ...
  // the 9pmÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“9am SAST auto schedule.
  ...
  // Realtime rather than a poll ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â an admin toggling this on one device
  ...
  // Admin-only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every account on the platform, for the Accounts screen.
  ...
  // each account's Google sign-in email from auth.users ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a table normal
  ...
  // Admin-only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same shape as loadAccounts above: routed through a
  ...
  // Admin-only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â permanently deletes an account (login, profile, phone,
  // membership rows) via the admin_delete_account() SQL function. Anything a
  // running league already shows ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â club names, fixtures, results ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â is
  ...
  const deleteAccount = (account, leagueCounts) => {
    ...
    const createdWarning = leagueCounts.created > 0
      ? ` They created ${leagueCounts.created} league${leagueCounts.created === 1 ? "" : "s"} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${leagueCounts.created === 1 ? "it" : "those"} will keep running exactly as-is, just manageable only by platform admins from now on.`
      : "";
    const joinedWarning = leagueCounts.joined > 0
      ? ` They're a member of ${leagueCounts.joined} league${leagueCounts.joined === 1 ? "" : "s"} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â their club name and results stay in those leagues, just no longer linked to a live account.`
      : "";
    requestConfirm([
      ...
      `Last check before deleting ${label} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â still want to continue?`,
      `Final confirmation ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to permanently delete ${label}'s account.`,
    ], async () => {
  ...
  // Admin-only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â marks an account approved via a security-definer function
  ...
  // picker ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â just enough to browse and pick someone (username + photo), never
  ...
  // Every earned badge from every member ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the Wall of Fame's raw material.
  ...
  // The permanent ladder ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every member, ordered by rank_position. Never
  ...
  // logging error) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â callers fall back to a plain confirmation toast.
  const describeLadderOutcome = async (source, sourceId) => {
    ...
    if (!data.applied) {
      ...
      if (data.reason === "pair_cooldown") return "you two already have 2 ladder results today ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this one's just for the record.";
      return null;
    }
  ...
  // leave you with nobody to challenge ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â for that first match only, clubs
  ...
  // Lets a member stop receiving new ladder challenges ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â e.g. if they're
  ...
  const toggleLadderPause = async () => {
    ...
    await loadLadder();
    showToast(next ? "Ladder challenges paused ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you won't receive new ones until you unpause." : "Ladder challenges resumed.");
  };
  ...
  // else in the app ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a team's display_name/phone are snapshotted at join
  // time too) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the opponent's phone stays off the row entirely until they
  ...
  const sendChallenge = async (opponent, isLadder = false) => {
    ...
    showToast(isLadder ? `Ladder challenge sent to ${opponent.username} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â win it and their spot is yours.` : `Challenge sent to ${opponent.username}.`);
  };

  // Accepting fills in the opponent's own phone right at the moment they agree
  // to it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the only way their number ever lands on the row. Declining just
  ...
  const respondChallenge = async (challenge, accept) => {
    ...
    await loadChallenges();
    showToast(accept ? `Challenge accepted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â say hi on WhatsApp.` : "Challenge declined.");
  };

  // Withdraws a still-pending challenge (challenger's side), or clears a
  // declined/accepted one off the list once it's been seen ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â either way just
  ...
  // Either side of an accepted challenge can log the score first ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it lands as
  ...
  const reportChallengeResult = async (challenge, myScore, theirScore, rawFile) => {
    ...
    await loadChallenges();
    showToast("Result logged ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â waiting for them to confirm.");
  };

  // The player who *didn't* report the score confirms it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this is enforced
  ...
  const confirmChallengeResult = async (challenge) => {
    ...
    showToast(outcome ? `Result confirmed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${outcome}` : "Result confirmed.");
  };
  ...
  const viewChallengeResultProof = async (challenge) => {
    ...
    // New rows store a permanent Blob URL directly ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â open it as-is. Rows
  ...
  const disputeChallengeResult = async (challenge) => {
    ...
    await loadChallenges();
    showToast("Result disputed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ask them to re-log it.");
  };

  // Admin-only fallback once challengeResultConfirmExpired(challenge) is true ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  const adminApproveChallengeResult = async (challenge) => {
    ...
    await loadLadder();
    const outcome = await describeLadderOutcome("challenge", challenge.id);
    showToast(outcome ? `Result approved ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${outcome}` : "Result approved.");
  };
  const adminRejectChallengeResult = async (challenge) => {
    ...
    await loadChallenges();
    showToast("Result rejected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â they'll need to log it again.");
  };

  // Admin-only fallback once a ladder challenge's 5-day accept window has
  // passed with no response ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â an admin can grant the challenger a walkover
  ...
  const adminGrantLadderWalkover = async (challenge) => {
    ...
    await loadLadder();
    const outcome = await describeLadderOutcome("challenge", challenge.id);
    showToast(outcome ? `Walkover granted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${outcome}` : "Walkover granted.");
  };
  // The other admin option for the same queue ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â closes the challenge out
  ...
  // logged results from every member on the platform ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â both confirmed and
  ...
  // toast ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this feed is a nice-to-have, not worth interrupting anyone) so
  ...
  // Comment wall shown under Challenges ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a single platform-wide board (not
  ...
  // down with it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â replies nest to unlimited depth, so this counts every
  ...
  const deleteBoardComment = (comment) => {
    ...
    requestConfirm([
      ...
      `Final check ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to permanently delete this ${noun}.`,
    ], async () => {
  ...
  // The Ladder's own comment wall ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same shape and behavior as the challenge
  ...
  // Last 100 confirmed ladder-challenge results, platform-wide ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reads from
  ...
  const loadLadderResults = useCallback(async () => {
    ...
    // a 120s single-click signed link) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never in this public recent-
  ...
  // Fires one challenge open to every other member. Anyone can grab it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // row lock lets exactly one of these UPDATEs through ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the loser gets 0
  ...
  const acceptOpenChallenge = async (challenge) => {
    ...
    if (!data || data.length === 0) { showToast("Too slow ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â someone else already accepted that one."); await loadOpenChallenges(); return; }
    await loadOpenChallenges();
    showToast(`Challenge accepted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â say hi on WhatsApp.`);
  };
  ...
  // Same report ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ confirm/dispute flow as reportChallengeResult, on the
  // open_challenges table instead ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â scores are stored from the creator's
  ...
  // challenge gets confirmed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so refresh it quietly while Home is open,
  ...
  // own actions (accept, decline, log a result, etc.) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â if the *other*
  ...
  // While the Challenges screen ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â or Home, where the random-challenge
  // notification banner lives ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â is open, poll the random-challenge pool
  ...
  // Same idea for the community results feed, on a slower clock ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â new
  ...
  useEffect(() => {
    ...
    if (linkedId) {
      ...
      else showToast("That league link isn't accessible ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you may need to be added as a member first.");
      window.history.replaceState({}, "", window.location.pathname);
    }
    setHandledDeepLink(true);
  }, [leagues, handledDeepLink, showToast]);

  // Handle a shared shop product link like /shop/<id> ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â anyone can open
  ...
  useEffect(() => {
    document.title = view === "shop" ? "Department Store" : "Matchday ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â eFootball Leagues";
  }, [view]);

  // Push a real browser history entry for every screen change, so the
  // hardware/gesture back action moves between in-app screens (League ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢
  // Home, Shop ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ Home, etc.) instead of leaving the site entirely ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â which
  ...
  useEffect(() => {
    const onPopState = (e) => {
      const state = e.state;
      if (!state || !state.appView) return; // not one of ours ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â leave it to whichever nav owns it
  ...
  // Every in-app "ÃƒÂ¢Ã¢â‚¬Â Ã‚Â Back" button used to call setView("home") directly ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  const completeProfile = async (phone, username, age, photoFile) => {
    ...
    if (error) {
      if (error.code === "23505" && error.message.toLowerCase().includes("phone")) {
        showToast("That phone number is already linked to another account ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â double-check it, or use a different number.");
      } else {
        showToast("Couldn't save your details ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try again.");
      }
      return;
    }
    setProfile(data);
  ...
  // Lets a signed-in member fix their own phone/username later ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the only way to
  ...
  // becomes the active one ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â if any entry's 24h window has lapsed with no
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!error) {
        ...
        // rather than an explicit decline ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â doesn't change what happened,
  ...
  // ladder_cup_entries row ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â e.g. this league's pre-listed clubs were added
  ...
  // the second-life expiry above ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fires once per league, the first time
  ...
  // someone next opens the league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â running this on read rather than on a
  // timer costs nothing.
  //
  ...
  // finalizedMatches/finalizedClaims aren't used to change anything ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â per
  // its own doc comment they never undo points already applied ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â they're
  ...
  useEffect(() => {
    ...
    // lapsed-but-not-yet-converted second-life offer sitting in it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the
    ...
    // statuses, activeLeague changes and this effect runs again ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â that
    ...
    (async () => {
      ...
      // whichever member's browser got here first ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but the RLS UPDATE
      ...
      // this exact narrow update ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nothing else on `leagues` is opened up.
      ...
      if (!wonRace || wonRace.length === 0) {
        ...
        // check above and this write ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nothing left for us to do.
        await refreshLeague(activeLeague.id);
        return;
      }
      // The RPC sets this server-side (now()) rather than us passing a
      // client-side timestamp ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â read it back off the returned row so the
      ...
      if (champion) {
        ...
        if (champRow) {
          // Bug fix: this write was previously unchecked ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a failure here
      ...
      let announcement = championName
        ? `Ladder Cup cutoff reached ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${championName} crowned champion with ${champion.pts} pts.`
        : "Ladder Cup cutoff reached ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no eligible champion (every club was eliminated).";
      const droppedBits = [
  ...
  // "Find your opponent" flow uses ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â pre-filled with that exact fixture, so
  ...
  useEffect(() => {
    ...
    if (!fixture) return; // not loaded into this league's data yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â wait for the next update
    setPendingLogFixtureId(null);
    if (fixture.played) return; // already logged elsewhere in the meantime ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â just land on the league
    if (isFixtureLocked(fixture, activeLeague)) {
      showToast("That match passed its 2-day deadline without a result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â both clubs received a loss. It's no longer loggable.");
      return;
    }
  ...
  // but doesn't by itself count as having joined ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the creator/admin can
  ...
  // no-show tie ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see findNoShowTeamIds. This is what makes the cut
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // finalizeGroups: don't just trust the update call succeeded ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â count
      ...
      if (updatedIds.size === 0) return; // every row blocked ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nothing changed, nothing to reload or announce

      // Drop an auto-posted comment in each affected league's feed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the
      // same "isResult" system-comment mechanism already used for
      // auto-posted matchday results ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so the eliminated club sees it
      ...
      for (const { league, ids } of targets) {
        ...
        const body = names.length === 1
          ? `${names[0]} was automatically eliminated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â missed a match past its deadline, and the no-show penalty already put them at -4 on it.`
          : `${names.join(", ")} were automatically eliminated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â missed a match past its deadline, and the no-show penalty already put them at -4 on it.`;
      ...
      if (updatedCount < allIds.length) {
        showToast(`${updatedCount} of ${allIds.length} no-show clubs${where} were auto-eliminated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the rest hit a permissions issue and will retry next reload.`);
        return;
      }
      showToast(`${updatedCount} club${updatedCount === 1 ? "" : "s"} eliminated automatically${where} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no-show on a match past its deadline.`);
    })();
  ...
  // Ladder Cup has no separate entry-close date of its own ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â clubs can join
  // right up until the ladder's weekly cutoff ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so entry_closes_at is
  ...
  // marker only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â clubs keep registering right up to the cutoff/finalize,
  ...
  const createLeague = async (input) => {
    ...
    const insertPayload = {
      ...
      // Survival Ladder Cup has no entry-close date of its own ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â clubs join
      // until the ladder's own cutoff, not a separate registration window ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
      ...
      // Drives the guest homepage's Weekend League spotlight ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â only leagues
      ...
      // Only an admin can actually create a cash league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â enforced again here
    ...
    // format, fixtures are NOT generated yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the league stays open for
    ...
    // pre-listed here or self-joined later ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see ensureLadderCupEntry.
    if (teamNames.length >= 2) {
      ...
      if (format === "ladder_cup") {
        // Same RLS-safe RPC as ensureLadderCupEntry ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a raw bulk insert here
      ...
      showToast(format === "ladder_cup"
        ? `League created ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${teamNames.length} club${teamNames.length === 1 ? "" : "s"} pre-listed and live on the ladder now. More clubs can join until entry closes.`
        : `League created ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${teamNames.length} club${teamNames.length === 1 ? "" : "s"} pre-listed. Review the list, then start the league when ready.`);
    } else {
      showToast(format === "ladder_cup"
        ? "League created ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â open for registration. Clubs are live on the ladder the moment they join."
        : "League created ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â open for registration. Players can join, then you can start it.");
    }
  ...
  const generateFixtures = async (league) => {
    ...
    if (teamsErr) { showToast("Couldn't confirm the current club list ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try again."); return; }
    ...
    await refreshLeague(league.id);
    showToast(`League started ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${fixtureRows.length} fixtures generated for ${freshTeams.length} clubs${groupAssignments ? ` across ${groupAssignments.length} groups` : ""}.`);
    } finally {
  ...
  const advanceGroupsToKnockout = async (league) => {
    ...
    // admin's browser already had ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a stale copy here can both cut the
    ...
    if (freshErr || !fresh) { showToast("Couldn't confirm the latest results ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try again."); return; }
    ...
    for (let g = 0; g < groupsCount; g++) {
      ...
      // A club auto-eliminated mid-group-stage (findNoShowTeamIds ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no-show
      ...
      // be a qualifier ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â bug: without this filter, an already-eliminated
    ...
    if (eliminatedIds.length > 0) {
      ...
      if ((updatedRows?.length || 0) < eliminatedIds.length) {
        showToast(`Only ${updatedRows?.length || 0} of ${eliminatedIds.length} clubs were actually eliminated (permissions issue) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â groups NOT finalized. Try again or check with support.`);
        return;
      }
    }

    // Pass the outer `league` (not `fresh`) here ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â knockoutBracketFixtures
    ...
    await refreshLeague(league.id);
    showToast(`Knockout stage started ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${qualifiers.length} clubs through.`);
  };

  const joinInFlight = useRef(new Set());
  // Same idea as joinInFlight, for the admin-side actions that generate or
  // advance fixtures ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a double-tap here (easy to do on mobile) would fire
  ...
  // fixtures-based formats) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a club is live in the ladder the moment its
  ...
  // table insert ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â regular authenticated clients don't have INSERT
  ...
  // a safe no-op ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no error code to swallow client-side anymore.
  ...
  // Ladder's `challenges` table does ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Ladder Cup has no accept/decline
  // step (see the ruleset: matching is by ladder-points band, not mutual
  // consent) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so this goes straight to a `ladder_cup_matches` row with
  ...
  const initiateLadderCupMatch = async (league, myTeamId, opponentTeamId) => {
    ...
    if (hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) { showToast("The Ladder Cup cutoff has passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no new matches."); return; }
    // Belt-and-suspenders against a double-tap creating two rows for the
    // same pairing before refreshLeague's response lands ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the DB has no
    ...
    // insert ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same RLS-safe pattern as ensure_ladder_cup_entry (see
    ...
    // call is gone ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â its result would have no bearing on what's actually
    // inserted.
    ...
    if (error) {
      ...
      // fresh as this league's last refreshLeague ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â if a match was created
    ...
    showToast(home === myTeamId
      ? "You're home ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â pick your match length to lock it in."
      : "Challenge sent ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â waiting on them to pick a match length.");
  };

  // Home team's choice, 6ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“15 minutes per half ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â isValidMatchLength mirrors
  ...
  const setLadderCupMatchLength = async (league, match, minutes) => {
    if (hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) { showToast("The Ladder Cup cutoff has passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this match no longer counts."); return; }
    if (!isValidMatchLength(minutes)) { showToast("Match length has to be 6ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“15 minutes."); return; }
    ...
    await refreshLeague(league.id);
    showToast(`Match length set ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${minutes} min a side. Go play it.`);
  };

  // Either side can back out of a match that hasn't had its length set yet
  // (no result exists to protect at that point ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â recordLadderCupWin only
  ...
  // of fields rendering needs ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â recordLadderCupWin needs the complete
  ...
  // writes results back. badge_walkover isn't part of the round trip ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  const ladderCupEntryFromRow = (row, clubName) => ({
    ...
    // Separate from pts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see formats/ladderCup.js. Falls back to the
  ...
  // Routed through apply_ladder_cup_entry_result (RPC, security definer ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // UPDATEs too ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â which is why confirmed results were silently failing to
  ...
  // around ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see the badge_walkover comment at the approve-claim call
  // site below) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â defaults to the row's existing count so ordinary result
  ...
  // Step 10: result logging ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â now a proper submit -> opponent
  ...
  //
  // submitLadderCupMatchResult (either side, once) reports a scoreline ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // (see 20260818_ladder_cup_result_pipeline.sql) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nothing here touches
  ...
  const submitLadderCupMatchResult = async (league, match, teamId, { homeGoals, awayGoals, extraTimeHomeGoals, extraTimeAwayGoals, pensHome, pensAway, file }) => {
    if (hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) { showToast("The Ladder Cup cutoff has passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this result can't be logged."); return false; }
    if (!file) { showToast("Attach a photo of the final scoreboard before saving."); return false; }
    if (!teamId) { showToast("Couldn't tell which club you're logging this for ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try refreshing."); return false; }
    ...
    if (error) {
      ...
      // earlier in this flow ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â someone else's report already landed while
      ...
      if (/already been reported/i.test(error.message)) {
        await refreshLeague(league.id);
        showToast("They already logged a result for this match ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â check it below.");
        return false;
      }
    ...
    await refreshLeague(league.id);
    showToast("Result logged ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â waiting for them to confirm or dispute it.");
    return true;
  };
  ...
  // (adminResolveLadderCupMatchResult) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â those are the only two ways a
  ...
  const applyLadderCupMatchResult = async (league, match) => {
    ...
    if (!winnerRow || !loserRow) { showToast("Couldn't find both clubs' ladder entries ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try refreshing."); return false; }
    ...
    // definer ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see supabase/migrations/20260820_ladder_cup_match_admin_rpc.sql)
    ...
    // ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but an admin resolving an *escalated* match (timeout or dispute
    ...
    // 20260819's migration) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â just without a visible fallback toast,
    ...
    let scoreLine = `${homeName} ${match.home_goals} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${match.away_goals} ${awayName}`;
    ...
    await postComment(league, `Ladder Cup ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${scoreLine}`, null, null, match.proof_url, true, null, null, match.id);

    await refreshLeague(league.id);
    showToast(loser.status === "eliminated"
      ? `Result confirmed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${teamsById[loserTeamId]?.name || "they"} are eliminated.`
      : loser.status === "pending_second_life"
      ? `Result confirmed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${teamsById[loserTeamId]?.name || "they"} have 24h to accept a second life.`
      : "Result confirmed.");
    return true;
  };

  // Wipes a reported-but-unconfirmed result back to scratch ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same shape
  ...
  // there's nothing to unwind there ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â only the reported fields need
  // clearing.
  const clearLadderCupMatchResult = async (league, match) => {
    // Same fix as confirm_ladder_cup_match_result just above ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a plain
  ...
  // The player who *didn't* report the score confirms or disputes it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // client-side (see LadderCupOpponentRow) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â from there it's admin-only
  ...
  const respondLadderCupMatchResult = async (league, match, teamId, accept) => {
    ...
    if (!ok) { showToast("Couldn't dispute the result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try refreshing."); return; }
    await refreshLeague(league.id);
    showToast("Result disputed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ask them to re-log it.");
  };
  ...
  // resolved) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same two outcomes as the opponent's own confirm/dispute,
  ...
  const adminResolveLadderCupMatchResult = async (league, match, approve) => {
    ...
    const ok = await clearLadderCupMatchResult(league, match);
    if (!ok) { showToast("Couldn't reject the result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try refreshing."); return; }
    await refreshLeague(league.id);
    showToast("Result rejected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â they'll need to log it again.");
  };

  // Step 11: second-life accept/decline. Either action ends the 24h window
  // immediately ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â accept re-enters the ladder at -6 points (floored at 0,
  ...
  // (as far as this function's concerned) is indistinguishable from one ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  const respondLadderCupSecondLife = async (league, teamId, accept) => {
    ...
    if (hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) { showToast("The Ladder Cup cutoff has passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â second life offers are closed."); return; }
    const teamsById = Object.fromEntries((league.teams || []).map((t) => [t.id, t]));
    const row = (league.ladder_cup_entries || []).find((r) => r.team_id === teamId);
    if (!row) { showToast("Couldn't find your ladder entry ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try refreshing."); return; }
    ...
    // ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â without this, a future correction to an earlier match couldn't tell
    ...
    await refreshLeague(league.id);
    showToast(accept ? `Back in it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â re-entered at ${updated.pts} pts.` : "Second life declined ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you're eliminated from this cup.");
  };

  // Step 12: walkover claims (message ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ 24h wait ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ claim with screenshot ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢
  // admin review). "Message opponent" is a purely local bookkeeping step ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  // the actual message happens outside the app (WhatsApp) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â createWalkoverClaim
  ...
  const messageLadderCupWalkoverOpponent = async (league, myTeamId, opponentTeamId) => {
    if (!myTeamId || !opponentTeamId) return;
    if (hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) { showToast("The Ladder Cup cutoff has passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no new walkover claims."); return; }
    ...
    // direct insert ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same RLS-safe pattern as ensure_ladder_cup_entry and
    ...
    await refreshLeague(league.id);
    showToast("Opponent messaged ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â claimable in 24h if they still haven't played.");
  };
  ...
  const submitLadderCupWalkoverClaim = async (league, claimRow, file) => {
    if (hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) { showToast("The Ladder Cup cutoff has passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this claim no longer counts."); return; }
    if (!file) { showToast("Attach a screenshot before submitting the claim."); return; }
    if (!isWalkoverClaimable(claimRow)) { showToast("Not claimable yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â still inside the 24h wait."); return; }
    ...
    await refreshLeague(league.id);
    showToast("Walkover claim submitted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â waiting on admin review.");
  };

  // Admin approval: flips the claim, then applies the walkover exactly the
  // way LADDER_CUP_INTEGRATION.md's own sample does ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â recordLadderCupWin
  ...
  const approveLadderCupWalkoverClaim = async (league, claimRow) => {
    if (hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) { showToast("The Ladder Cup cutoff has passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this claim can no longer be approved."); return; }
    try {
    ...
    if (!winnerRow || !loserRow) { showToast("Couldn't find both clubs' ladder entries ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try refreshing."); return; }
    ...
    await postComment(league, `Ladder Cup ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â walkover win for ${winnerName} over ${loserName}`, null, null, claimRow.proof_url, true, null, null);

    await refreshLeague(league.id);
    showToast("Walkover approved ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â result applied to the ladder.");
  };
  ...
  const joinLeague = async (leagueId) => {
    ...
    if (league.format === "ladder_cup" && hasLadderCupCutoffPassed(league.ladder_cup_cutoff_at)) {
      showToast("This Ladder Cup has already reached its cutoff ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no new clubs can join.");
      return;
    }
    ...
    if (league.league_type === "fun") {
      ...
      if (activeFunLeague) {
        showToast(`You're still active in "${activeFunLeague.name}" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â join another ${formatKindLabel(league.format)} league once your club there is eliminated, or that league finishes.`);
        return;
      }
    }
    ...
    if (match) {
      ...
      if (alreadyClaimed) {
        showToast(`"${match.name}" is already claimed by another member in this league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â contact the league admin.`);
        return;
      }
    } else if (!started) {
      ...
      if (teamErr) {
        if (teamErr.code === "23505") {
          showToast(`"${profile.efootball_username}" is already registered in this league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â contact the league admin if that's a mistake.`);
        } else {
    ...
    // self-registered teams (the branch above) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a team claimed by name
    ...
    if (error) { showToast("Couldn't join ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you may already be a member."); return; }
    logActivity("league_joined", { league_id: leagueId, league_name: league.name, as_team: !!match });
    await refreshLeague(leagueId);
    showToast(match ? `Joined ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you're playing as ${match.name}.` : "Joined as a spectator ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â your username isn't on this league's team list.");
    } finally {
  ...
  const claimOrRegisterTeam = async (league) => {
    ...
    if (match) {
      ...
      // See the matching comment in joinLeague ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a claimed pre-listed team
  ...
  // private storage, and creates the member row with payment_status "pending" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  const joinCashLeague = async (league, fee, rawFile) => {
    ...
    if (error) { showToast("Couldn't submit registration ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you may already be a member."); return false; }

    await refreshLeague(league.id);
    showToast(`Registration submitted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${formatRand(feeNum)} pending admin approval.`);
    return true;
  };
  ...
  const resubmitCashPayment = async (league, member, fee, rawFile) => {
    ...
    await refreshLeague(league.id);
    showToast(`Resubmitted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${formatRand(feeNum)} pending admin approval.`);
    return true;
  };
  ...
  // Admin/creator only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â downloads via a short-lived signed URL since the bucket is private.
  ...
  const applyPaymentReview = async (member, status) => {
    ...
    showToast(status === "approved" ? `Payment approved ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${member.display_name} is confirmed.` : `Payment marked as rejected for ${member.display_name}.`);
  };
  ...
  const reviewPayment = (member, status) => {
    ...
    requestConfirm([
      ...
      `Last check before rejecting ${member.display_name} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â still want to continue?`,
      `Final confirmation ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to reject ${member.display_name}'s club.`,
    ], () => applyPaymentReview(member, status));
  };

  // Fired when an admin taps the WhatsApp icon next to a member ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â flags that
  // member red (for every admin) for WA_REMINDER_WINDOW_MS. Just "someone
  // messaged them recently" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not tied to a fixture due date, so it fires
  ...
  // write is silently cut off mid-flight ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same code, purely device/browser
  ...
  // since keepalive requests cap out at 64KB ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fine for this tiny patch, but
  ...
  // held by this component ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never `await supabase.auth.getSession()` here.
  ...
  // synchronously keeps this to exactly one network call ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the keepalive
  // one ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â instead of stacking a second, unprotected one in front of it.
  const markWaReminder = async (member) => {
    const token = session?.access_token;
    if (!token) { console.warn("[wa-reminder] skipped ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no session token"); return; }
    ...
    // away ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the browser tab can get backgrounded mid-request, which can cut
    ...
    try {
      ...
      // TEMP DEBUG ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â remove once confirmed working. keepalive responses can't
      ...
      // even if this network call got cut off by the app handoff ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no toast
  ...
  // isWaReminderActive above) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â e.g. once the admin knows the member has
  ...
  // Bulk version of clearWaReminder ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â clears every currently-highlighted
  // member in one league at once, e.g. after a round of messaging is done
  // and the admin wants a clean slate rather than clicking each ÃƒÆ’Ã¢â‚¬â€ one at
  // a time.
  ...
  // their own call) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but a photo of the final scoreboard is required here
  ...
  // Shared by every path that can finish a knockout-bracket fixture ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  const applyKnockoutElimination = async (league, fixture, homeScore, awayScore, pensHome = null, pensAway = null) => {
    ...
    // Fetch this tie's leg(s) fresh ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not from local `league.fixtures` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
    ...
    // the final, a penalty score ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â if one's been entered ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â decides it now.
    ...
    if (winnerId) {
      ...
      // outcome ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not just marking the loser eliminated. This matters
      ...
      if (elimLoserErr || elimWinnerErr) showToast("Result saved, but a club's elimination status couldn't be fully updated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â check permissions.");
    }
  };
  ...
  const recordResult = async (league, fixture, homeScore, awayScore, file = null, pensHome = null, pensAway = null) => {
    ...
    await postComment(league, `Matchday ${fixture.round} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${homeName} ${homeScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${awayScore} ${awayName}`, null, file, null, true, null, fixture.id);
    await refreshLeague(league.id);
    ...
    showToast(outcome ? `Saved: ${homeName} ${homeScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${awayScore} ${awayName} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${outcome}` : `Saved: ${homeName} ${homeScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${awayScore} ${awayName}`);
  };
  ...
  const submitMatchResult = async (league, fixture, homeScore, awayScore, rawFile, pensHome = null, pensAway = null) => {
    ...
    if (error) {
      if (error.code === "23505") showToast("Someone already submitted a result for this match ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it's waiting on their opponent (or an admin) to review.");
    ...
    await refreshLeague(league.id);
    showToast("Result submitted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â pending admin approval.");
    return true;
  };
  ...
  // Admin/creator only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â downloads a submitted result's photo proof via a
  ...
  // Approving/rejecting both go through security-definer SQL functions ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // support though, so its own auto-posted comment is text-only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this adds
  ...
  const approveResult = async (league, submission) => {
    ...
    // only copies the score into the fixtures row ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it doesn't know about
    ...
    if (submission.photo_path) {
      const homeName = league.teams.find((t) => t.id === fixture?.home_team_id)?.name || "Home";
      const awayName = league.teams.find((t) => t.id === fixture?.away_team_id)?.name || "Away";
      // New rows already hold a permanent Blob URL ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use it directly. Rows
      ...
      if (photoUrl) {
        await postComment(
          league,
          `Photo proof for ${submission.submitted_by_username}'s approved result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Matchday ${fixture?.round} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${homeName} ${submission.home_score} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${submission.away_score} ${awayName}`,
    ...
    showToast(outcome ? `Result approved ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â posted to comments as ${submission.submitted_by_username} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${outcome}` : `Result approved ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â posted to comments as ${submission.submitted_by_username}.`);
  };
  ...
  const rejectResult = (league, submission) => {
    requestConfirm([
      ...
      `Final check ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to reject this result.`,
    ], async () => {
      ...
      // photo ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not just the admin. result-proofs is a private bucket (unlike
      ...
      let photoUrl = null;
      if (submission.photo_path) {
        // New rows already hold a permanent Blob URL ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use it directly.
      ...
      await postComment(
        league,
        `${submission.submitted_by_username}'s result was rejected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Matchday ${fixture?.round} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${homeName} ${submission.home_score} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${submission.away_score} ${awayName}`,
        null, null, photoUrl, true,
      );

      await refreshLeague(league.id);
      showToast("Result rejected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â posted to comments.");
    });
  };

  // The opponent's side of a pending submission ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same two outcomes as the
  ...
  // identity ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â since they're the one actually clicking the button, that
  ...
  const respondToResultSubmission = (league, submission, accept) => {
    const post = async () => {
      ...
      await postComment(
        league,
        accept
          ? `Matchday ${fixture?.round} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${homeName} ${submission.home_score} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${submission.away_score} ${awayName} (confirmed by opponent)`
          : `${submission.submitted_by_username}'s result was disputed by their opponent ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Matchday ${fixture?.round} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${homeName} ${submission.home_score} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${submission.away_score} ${awayName}`,
        null, null, photoUrl, true, null, accept ? (fixture?.id || null) : null,
      );

      await refreshLeague(league.id);
      showToast(accept ? "Result confirmed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â posted to comments." : "Result disputed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â they'll need to resubmit.");
    };
    ...
    requestConfirm([
      ...
      `Are you sure? The match will stay unplayed until someone resubmits.`,
      `Final check ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to dispute this result.`,
    ], post);
  };
  ...
  const advanceKnockout = async (league) => {
    ...
    // off stale local data here is the worst version of this bug ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it can
    ...
    // Whether this round IS the final ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see isFinalRoundFixtures. Only the
    ...
    // aggregate for the same reason both sides no-showed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nobody actually
    ...
    Object.values(ties).forEach((legs) => {
      ...
      if (totals[teamA] === totals[teamB]) {
        ...
        // The final always needs exactly one winner ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fall back to penalties.
    ...
    if (finalNeedsPens) { showToast("The final is level after regulation ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â enter the penalty shootout score to decide a winner."); return; }
    ...
    if (bothEliminatedIds.length > 0) {
      ...
      if ((updatedRows?.length || 0) < bothEliminatedIds.length) {
        showToast(`Only ${updatedRows?.length || 0} of ${bothEliminatedIds.length} no-show clubs were actually eliminated (permissions issue) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â round NOT advanced. Try again or check with support.`);
        return;
      }
    }
    if (winners.length <= 1) { showToast("This league already has a champion."); return; }

    // dueOffset: 1 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â dueBase here is "right now" (the moment this round is
    ...
    showToast(eliminatedTiesCount > 0
      ? `Round ${maxRound + 1} created. ${bothEliminatedIds.length} club${bothEliminatedIds.length === 1 ? "" : "s"} eliminated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no-show on both sides in ${eliminatedTiesCount} tie${eliminatedTiesCount === 1 ? "" : "s"}.`
  ...
  const advanceSurvivor = async (league) => {
    ...
    // cut ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not the `league` object already sitting in the browser's state.
    ...
    // against it can eliminate a club that had actually won its match ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
    ...
    if (fresh.final_stage_started) { showToast("This is the final stage ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â check the table for the champion."); return; }
    ...
    if (eliminatedIds.length > 0) {
      ...
      if ((updatedRows?.length || 0) < eliminatedIds.length) {
        showToast(`Only ${updatedRows?.length || 0} of ${eliminatedIds.length} clubs were actually eliminated (permissions issue) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â stage NOT advanced. Try again or check with support.`);
        return;
      }
    }
    ...
    await refreshLeague(league.id);
    showToast(goingFinal ? `Final stage started ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${remainingIds.length} clubs remain.` : `Stage ${nextStage} started ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${remainingIds.length} clubs remain.`);
  };
  ...
  const removeTeam = (team) => {
    requestConfirm([
      ...
      `Last check before removing ${team.name} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â still want to continue?`,
      `Final confirmation ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to permanently remove ${team.name}.`,
    ], async () => {
  ...
  // league hasn't started yet (fixtures.length === 0) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â once fixtures exist, wiping
  ...
  const leaveLeague = (league) => {
    ...
    requestConfirm([
      ...
      `Final check ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to leave "${league.name}" for good.`,
    ], async () => {
  ...
  // entry-close and kickoff dates back ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â plans change, a WhatsApp group is
  ...
  // the creator had a phone on their profile) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â without this, the "message
  ...
  const updateLeagueSchedule = async (league, { entryClosesAt, startsAt }) => {
    ...
    // above) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â LeagueScheduleLine doesn't even offer the field for this
  ...
  // once fixtures open, but only while the league hasn't started yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â once
  ...
  // "Due X" and never block a submission or auto-score a loss) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this date
  ...
  // no fixtures to generate (see generateFixtures ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â that's for the other
  // formats only) and clubs are already live on the ladder the moment they
  // join, so starting it doesn't create anything ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it just flips a status
  ...
  const startLadderCupLeague = async (league) => {
    ...
    try {
      ...
      await refreshLeague(league.id);
      showToast(`League started with ${league.teams.length} clubs on the ladder ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â new clubs can still join anytime before the cutoff.`);
    } finally {
      stageActionInFlight.current.delete(key);
    }
  };
  ...
  // for every member's WA icon in this league. Persists on the league row ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  const updateLeagueMemberMessage = async (league, text) => {
    ...
    await refreshLeague(league.id);
    showToast(text ? "Member message updated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â used for every WhatsApp nudge in this league from now on." : "Member message cleared ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â back to the default auto message.");
  };

  // Broadcasts the league's saved custom message to every member right
  // now, at no cost ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no SMS/WhatsApp Business API involved. It posts as
  ...
  // notification, not a push straight to someone's phone ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a member sees
  ...
  const notifyAllMembers = (league) => {
    ...
    requestConfirm([
      ...
      // No single member to derive {round}/{due} from for a broadcast ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use
      ...
      if (posted) {
        ...
        // (see markWaReminder) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a broadcast is still notifying every
        // member, so every member's row gets flagged too.
        (league.members || []).forEach((mm) => markWaReminder(mm));
        showToast(`Notified ${memberCount} member${memberCount === 1 ? "" : "s"} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â posted to the league feed.`);
      }
    });
  };

  // Comments live on every league regardless of stage ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â still filling up (pending)
  // or already generated fixtures (created/active) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so members can talk trash,
  ...
  // A comment or reply can optionally carry one photo ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â normally a fresh upload
  ...
  // re-uploading it).
  // voiceClip is { blob, duration } from useVoiceRecorder ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â optional, same
  ...
  // Admin-only correction for a posted result line (Results tab) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â edits the
  ...
  const editComment = async (comment, league, newBody) => {
    ...
    // silently ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â an update whose WHERE clause the policy filters out still
    ...
    if (!data) { showToast("Couldn't update ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you don't have permission to edit this result (check the comments UPDATE policy in Supabase)."); return false; }
    await refreshLeague(league.id);
  ...
  // (comment.fixture_id ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â only set on results posted after the fixture_id
  ...
  const editResultForFixture = async (comment, league, fixture, homeScore, awayScore) => {
    ...
    if (!fxData) { showToast("Couldn't update ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you don't have permission to edit this fixture (check the fixtures UPDATE policy in Supabase)."); return false; }

    await applyKnockoutElimination(league, fixture, homeScore, awayScore, fixture.pens_home, fixture.pens_away);

    const homeName = league.teams.find((t) => t.id === fixture.home_team_id)?.name || "Home";
    const awayName = league.teams.find((t) => t.id === fixture.away_team_id)?.name || "Away";
    const newBody = `Matchday ${fixture.round} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${homeName} ${homeScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${awayScore} ${awayName}`;
    ...
    await loadLadder(); // league results count toward ladder points, when eligible (see describeLadderOutcome)
    const outcome = await describeLadderOutcome("fixture", fixture.id);
    if (!cmError && cmData) showToast(outcome ? `Result updated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${outcome}` : "Result updated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â table refreshed.");
    return true;
  };

  // Ladder Cup's version of editResultForFixture ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but a Ladder Cup result
  ...
  // replay can't derive purely from the match/walkover event log ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â that's
  // a human decision, not a computed result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so it leans on
  ...
  // offer in its lifetime ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â that flag alone already records how it went.
  const computeLadderCupRecompute = (league) => {
    ...
    // Every club with an entry row today gets a fresh, zeroed-out seed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
    ...
    for (const m of (league.ladder_cup_matches || [])) {
      if (!m.finalized_at) continue; // never confirmed, or disputed away ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never happened
    ...
    for (const ev of events) {
      ...
      // Missing club (removed from the league since?) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â skip this one
      ...
      // can't have a next match in real life ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but if the corrected
      // timeline reshuffled who won an earlier match, that's exactly the
      // state we might be replaying into. Skip rather than feed a
      // pending_second_life club into another result ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same "can't be
      ...
      if (loser.status === "pending_second_life") {
        ...
        if (offer?.response_type === "accepted") {
          ...
          // No recorded response, but the 24h window's already lapsed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
          ...
          // else: no record either way and the row isn't eliminated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
          // treat as a genuinely still-open offer, same as the real-time case below.
        }
        // else: a real, still-open offer ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â leave the club pending_second_life.
      }
  ...
  // trip via bulk_apply_ladder_cup_entries (admin-only RPC ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see
  ...
  // (comment.ladder_cup_match_id) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see the recompute functions above for
  ...
  const editLadderCupMatchResult = async (comment, league, match, homeGoals, awayGoals) => {
    ...
    if (homeGoals !== awayGoals) {
      ...
      showToast("That scoreline is level ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this match has no extra time or penalties on record to decide it.");
      return false;
    }
    ...
    // Re-fetch rather than patching the local `league` object by hand ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
    ...
    if (fetchErr || !freshLeague) { showToast("Score corrected, but couldn't reload the league to recompute standings ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try refreshing."); return false; }
    const recomputeOk = await recomputeLadderCupLeague(freshLeague);

    const teamsById = Object.fromEntries((league.teams || []).map((t) => [t.id, t]));
    const homeName = teamsById[match.home_team_id]?.name || "Home";
    const awayName = teamsById[match.away_team_id]?.name || "Away";
    let scoreLine = `${homeName} ${homeGoals} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${awayGoals} ${awayName}`;
    if (decidedBy === "extra_time") scoreLine += ` (aet ${match.extra_time_home_goals}-${match.extra_time_away_goals})`;
    if (decidedBy === "penalties") scoreLine += ` (pens ${match.penalties_home}-${match.penalties_away})`;
    const { data: cmData, error: cmError } = await supabase.from("comments").update({ body: `Ladder Cup ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${scoreLine}` }).eq("id", comment.id).select().maybeSingle();
    if (cmError) showToast(`Score saved, but couldn't update the posted text: ${cmError.message}`);
    else if (!cmData) showToast("Score saved, but you don't have permission to edit the posted text (check the comments UPDATE policy).");

    await refreshLeague(league.id);
    if (recomputeOk && !cmError && cmData) showToast("Result corrected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the whole ladder was recomputed from scratch.");
    else if (recomputeOk) showToast("Standings recomputed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â but the posted text couldn't be updated.");
    return true;
  };
  ...
  // Likes are a simple toggle backed by a unique (comment_id, user_id) row ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // it's open to anyone signed in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not gated by canComment ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â since the
  ...
  // Suggestion box ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â open to anyone signed in, regardless of whether they've
  ...
  const postSuggestion = async (text) => {
    ...
    showToast("Thanks ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â suggestion sent!");
    return true;
  };
  ...
  const deleteLeague = (league) => {
    requestConfirm([
      ...
      `Last check ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â click to permanently delete "${league.name}".`,
    ], async () => {
      const { error } = await supabase.from("leagues").delete().eq("id", league.id);
      if (error) { showToast(`Couldn't delete: ${error.message}`); return; }
      setView("home");
      setActiveLeagueId(null);
      setLeagues((prev) => (prev || []).filter((l) => l.id !== league.id)); // already gone server-side ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no need to refetch
  ...
  const shareLeague = (league) => {
    ...
    showToast("Invite link copied ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â share it with members who already have access.");
  };
  ...
  // this is "opening" the screen ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the effects that poll them already fire
...
// The signed-out homepage. This *is* the site's front door now ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â visitors can
...
// does on its own is offer Google sign-in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every actual action (joining a
...
function PublicHome({ c, theme, toggleTheme, accentKey, setAccent, onSignIn, onRequireAuth, initialShopProductId }) {
  ...
  // is picked from ACCENTS and lives in the app root now ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see the comment
  // by accentKey's useState in App() ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so whatever a guest picks here is
  ...
  // effects) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a guest opening the shop and swiping back should land on the
  ...
  useEffect(() => {
    document.title = shopOpen ? "Department Store" : "Matchday ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â eFootball Leagues";
  }, [shopOpen]);
  ...
  // to anon in Supabase) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â loaded once here and handed down as props so the
  ...
  // Every guest page load (every visitor, every refresh ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not gated behind
  ...
  //      columns its guest-tree consumer(s) actually read ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â traced through
  ...
  //      but against a SEPARATE query there ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â narrowing the guest-only
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch { /* corrupt/unavailable sessionStorage ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â just refetch */ }
      ...
      const [leaguesRes, teamsRes, fixturesRes, extraRes, ladderRes, resultsRes, teamAvatarsRes, settingsRes] = await Promise.all([
        ...
        // (computeStandings, isFixtureLocked/isGroupStageFixture,
        // StandingsPanel, GroupTables, WeekendLeagueSpotlight,
        // PublicLeagueCard, leagueGoalExtremes) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â these are the only
        ...
        // result_submissions joins, etc. ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â none of those are touched by the
        ...
        // pens_away are deliberately excluded ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nothing in the guest tree
        ...
        // this view ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see PublicLeagueCard / isCashLeague above.
        ...
        // here, and only reads these fields ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same component the signed-in
        ...
        // Club-owner photos for the standings tables below ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â team_id ->
        ...
        // (see isWeekendPauseHour) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â read-only here, guests never see the
      ...
      // rejection ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so Promise.all still "succeeds" and nextGuestData
      ...
      if (anyErrored) {
        ...
        } catch { /* storage full/unavailable ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not worth failing the page over */ }
      }
    })();
    return () => { cancelled = true; };
  }, []);
  ...
  // time ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so a guest landing on the page gets a different pair each time,
  ...
  // Guests only ever see non-cash leagues (see the "Leagues" section below) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // kick off fresh over the coming FriÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Sun, or already have unplayed matches
  // due in that window ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â sorted so whatever's happening soonest leads.
  ...
  // restricts every row this view returns to admin-created leagues ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it
  ...
  // Weekend League spotlight above ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so a weekend league gets one true home
  ...
  return (
    <div className="min-h-screen" style={{ background: c.bg, color: c.text, fontFamily: "'Barlow Condensed', 'Oswald', sans-serif" }}>
      {/* Sticky guest header ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same shell language as the signed-in Header,
      ...
      <main className="max-w-3xl mx-auto px-4 pb-24">
        ...
        {/* Compact HUD banner ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same shell the signed-in Home uses (emblem,
            live-season pulse, stat strip). No CTA button here on purpose:
            this is the "look, it's real and it's live" beat, not the sign-in
            beat ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the header covers anyone in a hurry, and the single strong
          ...
          <div className="relative flex items-center gap-3 px-4 py-3.5">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <div className="text-right font-mono leading-tight">
                <div className="font-bold text-sm" style={{ color: c.text }}>{guestData ? funLeagues.length : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“"}</div>
              ...
              <div className="text-right font-mono leading-tight">
                <div className="font-bold text-sm" style={{ color: c.text }}>{totalClubs || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“"}</div>
                <div className="text-[9px] uppercase tracking-wider" style={{ color: c.textFaint }}>clubs</div>
              </div>
              <div className="w-px h-7" style={{ background: c.border }} />
              <div className="text-right font-mono leading-tight">
                <div className="font-bold text-sm" style={{ color: c.text }}>{totalMatches || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“"}</div>
                <div className="text-[9px] uppercase tracking-wider" style={{ color: c.textFaint }}>played</div>
              </div>
            </div>
          </div>
          <div className="relative px-4 pb-3.5">
            <p className="font-body text-xs" style={{ color: c.textDim }}>Have a look around ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â everything below is live.</p>
          </div>
        </section>

        {/* Weekend League spotlight ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the most time-sensitive thing on the
        ...
        {/* Menu tiles ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â usable ones lead now (Ladder, Leagues both just
          ...
          {guestData && otherFunLeagues.length === 0 && (
            <section className="mt-8">
              <div className="border border-dashed rounded-xl p-8 text-center font-body" style={{ borderColor: c.borderStrong, color: c.textDim }}>
                No leagues running yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â sign in and start the first one.
              </div>
            </section>
          )}
        ...
        {/* The one strong CTA on the page ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â everything above was proof, this
...
// Time-boxed highlight of whatever's happening over the coming FriÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Sun ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
// leagues kicking off fresh, or leagues with matches already due ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â surfaced
...
function WeekendLeagueSpotlight({ items, weekendStart, weekendEnd, onCardClick, isJoined, override, isAdmin, onSetOverride, c }) {
  ...
  // Paused: counting down to the 9am SAST resume ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â unless the weekend
  ...
  // end. Live: counting down to whichever comes first ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the 9pm SAST pause
    ...
    <section className="relative mt-4 rounded-2xl overflow-hidden" style={{ background: `linear-gradient(120deg, ${c.accent}22, ${c.surface})`, border: `1px solid ${c.accent}55` }}>
      ...
      <div className="relative px-4 pt-3.5 pb-1 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded shrink-0" style={{ background: c.surfaceHover, color: c.text }}>
          {isPaused ? (
            <><Pause size={10} /> Paused Ãƒâ€šÃ‚Â· {isOverridden ? "admin override" : `${pausedTargetIsEnd ? "ends" : "resumes"} in ${countdownLabel}`}</>
          ) : isLiveNow ? (
            <>
              <span className="relative flex h-1.5 w-1.5">
                <span className="animate-pulse-dot absolute inline-flex h-full w-full rounded-full" style={{ background: c.accent }} />
              </span>
              Live Ãƒâ€šÃ‚Â· {isOverridden ? "admin override" : `${liveTargetIsEnd ? "ends" : "pauses"} in ${countdownLabel}`}
            </>
          ) : (
      ...
      <div className="relative px-4 pb-1.5 flex items-center gap-1.5 font-body text-xs" style={{ color: c.textDim }}>
        {isPaused
          ? "Overnight break ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â results can still be uploaded"
      ...
      {/* Admin-only manual override of the 9pmÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“9am auto pause/resume ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â for
...
// One equal-weight tile in the guest quick-action grid ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same visual as the
...
// The "sign in to continue" gate ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â shown over whatever the guest was just
...
// The ladder for guests ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â visually identical to the signed-in Home's
        ...
        {top5.map((row, i) => (
          <div key={row.user_id} className="flex items-center gap-2 shrink-0 rounded-xl pl-2 pr-3.5 py-2"
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="font-mono text-[10px]" style={{ color: c.textFaint }}>{row.points}pts Ãƒâ€šÃ‚Â· {row.wins}WÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“{row.losses}L</span>
            </div>
          </div>
        ))}
...
// Platform-wide "who just played" feed for guests ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reuses the exact same
...
// A section of league previews for guests ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same header format as the
...
function PublicLeagueCard({ league: l, data, onJoin, avatarByTeamId, c }) {
  ...
  // in PublicHome), so no cash badge on the header ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the description guard
        ...
        <div className="flex flex-col gap-1 mt-2.5 max-h-72 overflow-y-auto pr-0.5">
          {allLeagueFixtures.map((f) => (
            <div key={f.id} className="flex items-center justify-between gap-2 font-body text-xs px-2 py-1.5 rounded" style={{ background: c.surfaceHover }}>
              ...
              <span className="font-mono text-[11px] shrink-0" style={{ color: c.textFaint }}>{f.played ? `${f.home_score}ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“${f.away_score}` : "vs"}</span>
...
// Full-width promo banner for the WeAfrica Shop ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â sits above everything
  ...
  return (
    <section onClick={onOpen} className="relative mt-4 rounded-2xl overflow-hidden cursor-pointer active:scale-[0.98] transition-transform"
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        {!hasPicks && (
          /* Small "products" bobbing and glowing in the banner's corner ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
      ...
      <div className="relative flex items-center gap-3 px-4 py-3.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="font-body text-xs truncate" style={{ color: c.textDim }}>Kits, jerseys & gear ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â open to everyone</div>
        </div>
      ...
      {hasPicks && (
        ...
        // PublicHome) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â bobbing hexagon badges instead of generic icon
...
function ProfileGate({ c, theme, toggleTheme, onSubmit, userEmail, onSignOut }) {
  ...
  // Object URLs aren't garbage-collected automatically ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â without this, picking
  ...
  // having to reach for the button ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â there's no <form> here (this component
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: c.bg, color: c.text, fontFamily: "'Barlow Condensed', 'Oswald', sans-serif" }}>
      ...
      {userEmail && onSignOut && (
        <p className="font-mono text-[11px] text-center mb-6" style={{ color: c.textFaint }}>
          Signed in as {userEmail} Ãƒâ€šÃ‚Â·{" "}
      ...
      <div className="w-full max-w-sm">
        ...
        {usernameTrimmed.length > 0 && !usernameIsOneWord && (
          <p id="pg-username-error" className="font-body text-xs mb-1.5" style={{ color: c.red }}>No spaces ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use one word, like your actual in-game username (e.g. "Bounce_Academy" not "Bounce Academy").</p>
        )}
        <div className="mb-4" />
...
// Lets an already-onboarded member update their phone/username later ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â mainly the
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(0,0,0,0.6)" }}>
      <div className="w-full max-w-sm rounded-xl p-6" style={{ background: c.bg, border: `1px solid ${c.border}` }}>
        ...
        <div className="flex flex-col items-center mb-5">
          ...
          <span className="font-mono text-[10px] uppercase tracking-wider" style={{ color: c.textFaint }}>
            {uploadingPhoto ? "UploadingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : profile?.avatar_url ? "Change photo" : "Add profile photo"}
          </span>
        </div>
        ...
        <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="e.g. Ndosi_123"
          className="w-full border rounded-lg px-4 py-2.5 font-body outline-none mb-1.5" style={{ background: c.surface, borderColor: c.border, color: c.text }} />
        {usernameTrimmed.length > 0 && !usernameIsOneWord && (
          <p className="font-body text-xs mb-1.5" style={{ color: c.red }}>No spaces ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use one word, like your actual in-game username.</p>
        )}
        <div className="mb-4" />
...
// Admin-only raw activity feed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â step 1 of activity tracking (see
  ...
  return (
    <div className="pt-8">
      ...
      <div className="font-mono text-xs mb-5" style={{ color: c.textFaint }}>
        ...
        {pendingCount > 0 && <span style={{ color: "#B8860B" }}> Ãƒâ€šÃ‚Â· {pendingCount} pending approval</span>}
        {flaggedCount > 0 && <span style={{ color: c.red }}> Ãƒâ€šÃ‚Â· {flaggedCount} still need{flaggedCount === 1 ? "s" : ""} a phone number fixed</span>}
      </div>
...
// rows were handed in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reused by both the platform-wide Leaderboard and
    ...
    <div className={`grid ${least ? "grid-cols-2" : "grid-cols-1"} gap-2 mb-4`}>
      <div className="rounded-lg px-3 py-2.5 flex items-center gap-2.5 border" style={{ background: c.surface, borderColor: c.border }}>
        ...
        <div className="min-w-0">
          ...
          <div className="font-body text-xs font-semibold truncate" title={top.name}>{top.name} <span className="font-mono font-normal" style={{ color: c.textDim }}>Ãƒâ€šÃ‚Â· {top.gf}ÃƒÂ¢Ã…Â¡Ã‚Â½</span></div>
        </div>
      </div>
      {least && (
        <div className="rounded-lg px-3 py-2.5 flex items-center gap-2.5 border" style={{ background: c.surface, borderColor: c.border }}>
          ...
          <div className="min-w-0">
            ...
            <div className="font-body text-xs font-semibold truncate" title={least.name}>{least.name} <span className="font-mono font-normal" style={{ color: c.textDim }}>Ãƒâ€šÃ‚Â· {least.ga} conceded</span></div>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="rounded-lg px-4 py-2.5 flex items-center gap-3" style={{ background: c.surface }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="font-body text-sm truncate flex items-center gap-1.5">
          <span className="truncate">{account.efootball_username || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</span>
        ...
        {(leagueCounts.created > 0 || leagueCounts.joined > 0) && (
          <div className="font-mono text-[10px] mt-0.5" style={{ color: c.textFaint }}>
            ...
            {leagueCounts.created > 0 && leagueCounts.joined > 0 && " Ãƒâ€šÃ‚Â· "}
...
// Small round avatar used on the Challenges screen ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a photo if the member
...
export function MemberAvatar({ url, username, size = 32, c }) {
  if (url) {
    ...
    // URLs from costing Cached Egress on every view ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see mediaUrl.js.
...
// Shared emoji medal for a numeric rank ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â used by the standings, ladder and
...
// null for rank > 3 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â callers fall back to "#rank" text themselves.
export function medalFor(rank) {
  return rank === 1 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â¡" : rank === 2 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã‹â€ " : rank === 3 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â°" : null;
}

// Rating-card tier for a rank ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â turns the plain "top 3 get a ring" idea into
...
const CARD_TIERS = {
  gold: { label: "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â¡ GOLD CARD", ring: "#FFD700" },
  silver: { label: "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã‹â€  SILVER CARD", ring: "#C0C0C0" },
  bronze: { label: "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â° BRONZE CARD", ring: "#CD7F32" },
};
function tierFor(rank, c) {
  ...
  if (rank != null && rank <= 10) return { key: "rated", label: "ÃƒÂ¢Ã‚Â­Ã‚Â IN FORM", ring: c.accent };
...
// fixed prefix/suffix around it ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â "+3" animates the 3 and keeps the "+",
// "84%" animates the 84 and keeps the "%". Multi-number strings like
// "3 Ãƒâ€šÃ‚Â· 1 Ãƒâ€šÃ‚Â· 2" (W Ãƒâ€šÃ‚Â· D Ãƒâ€šÃ‚Â· L) don't match, so those stay static; animating three
...
// stagger delay ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â gives each stat tile its own little "reveal" instead of
...
// One stat tile on the player card ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a plain number ticks up from 0 and the
...
// eFootball-style rated player card ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reused by the Leaderboard, Ladder and
...
// fields (W/D/L, goals) or ladder fields (points, rank) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no extra data
...
// as a prop ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â callers used to compute their own medal copy and pass it in,
...
export function PlayerProfileModal({ username, avatarUrl, rank, isMe, stats, badges, onClose, c }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-0 sm:px-4" style={{ background: "rgba(0,0,0,0.65)" }} onClick={onClose}>
      ...
      >
        {/* Tier-tinted wash across the whole card ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â subtle for the rated/
        ...
        {/* Earned badges ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same defs the caller's own compact badge row
...
// the challenges they've sent or received. A challenge starts as "pending" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
function Header({ view, setView, activeLeague, theme, toggleTheme, c, onSignOut, userEmail, avatarUrl, onEditProfile, isAdmin, onOpenAccounts, onOpenActivity, onOpenChallenges, challengeBadge, onOpenSuggestion, onOpenLeaderboard, onOpenLadder, onOpenCreate, grabbableCount }) {
  ...
  useEffect(() => {
    ...
    // touchstart as well as mousedown ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â mousedown alone can fire late (or
  ...
  // Everything except Edit profile now lives in here ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â on a narrow phone
  ...
  // the edge was simply never reachable). Two buttons ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â profile + this
  // menu ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â always fit.
  const menuItems = [
      ...
      <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        {view === "league" && activeLeague && (
          <div className="hidden sm:block font-mono text-xs uppercase tracking-wider shrink-0" style={{ color: c.textFaint }}>
            {activeLeague.teams.length} clubs Ãƒâ€šÃ‚Â· {activeLeague.fixtures.filter((f) => f.played).length}/{activeLeague.fixtures.length} played
          </div>
        )}
...
// the website"). Open to any signed-in user ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â doesn't require joining or
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(0,0,0,0.5)" }} onClick={onCancel}>
      <div className="w-full max-w-sm rounded-xl p-5 border" style={{ background: c.bg, borderColor: c.borderStrong }} onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={submit} disabled={!text.trim() || posting}
          ...
          {posting ? "SendingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "Send suggestion"}
        </button>
      </div>
    </div>
  );
...
function Home({ leagues, isAdmin, isMemberOf, entryClosed, myPaymentStatus, canManageLeague, myTeam, onOpen, onCreate, onJoin, session, onToggleLeagueReaction, challenges, openChallenges, onOpenChallenges, onOpenLogResult, onOpenLogResultOpen, ladder, myLadderRank, onOpenLadder, onOpenLeaderboard, onOpenShop, memberAvatars, allAchievements, onAchievementsSynced, myAvatarUrl, weekendOverride, onSetWeekendOverride, showToast, c }) {
  ...
  // too so a signed-in player who hasn't joined yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â or who's a member of
  // an entirely different set of leagues ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â still sees what's kicking off
  // this FridayÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Sunday and can jump straight in with one tap instead of
  ...
  const weekendLeagues = funLeagues.filter((l) => l.created_by_admin).reduce((items, l) => {
    ...
    // not each match's own (now-advisory) due_at ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so if that shared deadline
  ...
  // Open random challenges anyone but the signed-in member can still grab ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // on either the challenge or open-challenge track ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â surfaced right at the
  ...
  // Same fun-league "active in this format-kind" map the join guard uses ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // Ordering priority: a league the player can actually join goes first ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // Achievement badges ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a second, more permanent collection layer next to
  ...
  // badge's value() function reads must be listed below ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â miss one and that
  ...
  // Every league title, grouped by who won it and which league/when ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â feeds
  ...
  // localStorage-per-user pattern the level-up toast above uses ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so a
  ...
  useEffect(() => {
    ...
    let seen = [];
    try { seen = JSON.parse(localStorage.getItem(key) || "[]"); } catch (e) { /* ignore ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â storage unavailable */ }
    ...
    if (newOnes.length > 0) {
      if (seen.length > 0 && showToast) {
        ...
        showToast(newOnes.length === 1 ? `Achievement unlocked: ${first.label} ÃƒÂ°Ã…Â¸Ã‚ÂÃ¢â‚¬Â ` : `${newOnes.length} new achievements unlocked ÃƒÂ°Ã…Â¸Ã‚ÂÃ¢â‚¬Â `);
      }
      try { localStorage.setItem(key, JSON.stringify(earnedIds)); } catch (e) { /* ignore ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â storage unavailable */ }
    }
  }, [myId, achievements, showToast]);

  // Wall of Fame ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every member's badge count/score, ranked. memberAvatars
  ...
  // Mirrors every earned badge to Supabase ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this is what lets a badge
  ...
  // ref, not state, so comparing it doesn't itself trigger a re-render) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
  ...
  // session never re-fires here on a later visit ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â only a level crossed
  ...
  useEffect(() => {
    ...
    let lastSeen = 0;
    try { lastSeen = Number(localStorage.getItem(key)) || 0; } catch (e) { /* ignore ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â storage unavailable */ }
    if (myProgress.level > lastSeen) {
      if (lastSeen > 0 && showToast) showToast(`Level up! You're now Lvl ${myProgress.level} Ãƒâ€šÃ‚Â· ${myProgress.levelTitle} ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Â°`);
      try { localStorage.setItem(key, String(myProgress.level)); } catch (e) { /* ignore ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â storage unavailable */ }
    }
  }, [myId, myProgress.level, myProgress.played, myProgress.levelTitle, showToast]);

  return (
    <div>
      {/* Player card ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â leads the page like a game's home dashboard: who's
      ...
      <section className="relative mt-1 rounded-2xl overflow-hidden" style={{ background: `linear-gradient(120deg, ${c.green}33, ${c.surface})`, border: `1px solid ${c.border}` }}>
        ...
        {/* Level + XP bar, with a streak chip when the player is on a run ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
        ...
        {myProgress.played > 0 ? (
          <div role="button" tabIndex={0} onClick={() => setProgressOpen(true)} onKeyDown={(e) => { if (e.key === "Enter") setProgressOpen(true); }}
            ...
            <div className="flex items-center gap-1 font-mono text-[10px] font-bold uppercase tracking-wider shrink-0 rounded-full px-2 py-0.5"
              ...
              <Star size={10} /> Lvl {myProgress.level} Ãƒâ€šÃ‚Â· {myProgress.levelTitle}
            </div>
      ...
      {/* Continue playing ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the two "something's waiting on you" strips,
      ...
      {/* Weekend League spotlight ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same admin-curated FriÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Sun highlight
      ...
      {/* Quick actions ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â one equal-weight action dock. Shop lives here as a
      ...
      {/* Achievements ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the badge collection layer, right after quick
      ...
      {/* Wall of Fame ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the shared, cross-player view of the same badges,
      ...
      {/* Where you stand ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Leaderboard preview then the Ladder banner,
      ...
      {leagues.length === 0 && (
        <section className="mt-6 rounded-2xl overflow-hidden" style={{ background: `linear-gradient(120deg, ${c.accent}22, ${c.surface})`, border: `1px solid ${c.border}` }}>
          <div className="flex flex-col items-center text-center gap-3 px-6 py-10">
            ...
            <div className="font-body text-sm max-w-[220px]" style={{ color: c.textDim }}>Start the first one ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it takes about a minute.</div>
          ...
          <div key={f.fixtureId} role="button" tabIndex={0} onClick={() => onOpen(f.leagueId, f.fixtureId)}
            ...
            <div className="flex items-center justify-between gap-1.5 mt-1.5">
              <div className="font-mono text-[10px] min-w-0 truncate" style={{ color: c.textDim }}>
                {f.isHome ? "Home" : "Away"}
                {f.expired ? <span style={{ color: c.red }}> Ãƒâ€šÃ‚Â· Expired</span> : f.due_at ? ` Ãƒâ€šÃ‚Â· Due ${fmtDate(f.due_at)}` : ""}
              </div>
              {f.opponent.phone && (
                // Stop the click from also bubbling up and opening the league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
                // tapping WhatsApp here should only open WhatsApp.
                <div onClick={(e) => e.stopPropagation()} className="shrink-0">
                  <WhatsAppCallLink phone={f.opponent.phone} iconOnly
                    text={`Hi, it's ${f.team.name} ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥ Call me when you're ready to play so we can lock in the time${f.due_at ? ` (due ${fmtDate(f.due_at)})` : ""} ÃƒÂ¢Ã…Â¡Ã‚Â½ÃƒÂ°Ã…Â¸Ã¢â‚¬Â¢Ã‚Â¹ÃƒÂ¯Ã‚Â¸Ã‚Â${firstMatchdayNote(f.round)}`} c={c} />
                </div>
              )}
            </div>
          </div>
...
// Sits at the very top of Home, above even "Up next" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â accepted challenges
...
// Small helper ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â first name off the signed-in user's email, purely cosmetic
...
// One equal-weight tile in the quick-action menu grid ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â icon on top, label
...
// The permanent ladder, sitting in front of everything else on Home ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a
...
// screen lives behind the header menu) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â top 5 by wins for the current
        ...
        <div className="relative flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div className="leading-tight">
              ...
              <div className="font-mono text-[9px] uppercase tracking-wider mt-0.5" style={{ color: c.textFaint }}>This season Ãƒâ€šÃ‚Â· every league</div>
            </div>
          </div>
          {myRow && (
...
// The picker for who a member is allowed to send a ladder challenge to ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
      ...
      <div className="w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-5" style={{ background: c.bg, color: c.text }} onClick={(e) => e.stopPropagation()}>
        ...
        {targets.length === 0 ? (
          <div className="font-body text-sm text-center py-6" style={{ color: c.textFaint }}>
            {myRank && myRank.rank_position === 1 ? "You're #1 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nobody left to challenge." : "No one within 10 points of you yet."}
          </div>
        ) : filtered.length === 0 ? (
...
// the Ladder or a league's standings table ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same columns, same colours as
...
// for whatever font is currently set on `ctx` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â canvas has no built-in
// text-overflow, so this is the manual equivalent.
function fitCanvasText(ctx, text, maxWidth) {
  if (ctx.measureText(text).width <= maxWidth) return text;
  let t = text;
  while (t.length > 1 && ctx.measureText(t + "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦").width > maxWidth) t = t.slice(0, -1);
  return t + "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦";
}
...
const SHARE_STANDINGS_COLUMNS = [
  ...
  { key: "name", label: "Club", width: 456, align: "left", isName: true, get: (r) => r.name + (r.eliminated ? " Ãƒâ€šÃ‚Â· OUT" : r.atRisk ? " Ãƒâ€šÃ‚Â· AT RISK" : "") },
...
// field on every row ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â position ranges are sliced 10 at a time off of it.
export function ShareRangeModal({ onClose, kicker, title, subtitle, rows, columns, c, defaultRank }) {
  ...
  const rangeLabel = `#${rangeStart}ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“${rangeEnd}`;
...
export const REACTIONS = [
  { key: "like", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ‚Â" },
  { key: "love", emoji: "ÃƒÂ¢Ã‚ÂÃ‚Â¤ÃƒÂ¯Ã‚Â¸Ã‚Â" },
  { key: "laugh", emoji: "ÃƒÂ°Ã…Â¸Ã‹Å“Ã¢â‚¬Å¡" },
  { key: "fire", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥" },
  { key: "wow", emoji: "ÃƒÂ°Ã…Â¸Ã‹Å“Ã‚Â®" },
  { key: "skull", emoji: "ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã¢â€šÂ¬" },
];
export const REACTION_EMOJI = Object.fromEntries(REACTIONS.map((r) => [r.key, r.emoji]));

// A reaction bar for the league itself ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same emoji-picker pattern as a
...
export function LeagueReactionBar({ league, session, onToggle, c, compact = false }) {
  ...
  // ever opening a league) as well as inside LeagueDetail ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â stopping
  ...
  return (
    <div className={compact ? "relative shrink-0" : "relative mb-5"} ref={pickerRef} onClick={(e) => e.stopPropagation()}>
      <button onClick={handleMainClick} disabled={!session}
        ...
        <span style={{ fontSize: compact ? 12 : 13, lineHeight: 1 }}>{myReaction ? REACTION_EMOJI[myReaction] : "ÃƒÂ°Ã…Â¸Ã‚Â¤Ã‚Â"}</span>
        {!compact && (myReaction ? "You reacted" : "React to this league")}
        {reactions.length > 0 && (
          <span>{compact ? "" : "Ãƒâ€šÃ‚Â· "}{summary.slice(0, 3).map(([key]) => REACTION_EMOJI[key]).join("")} {reactions.length}</span>
        )}
      </button>
    ...
    <div className="flex items-center gap-2.5 rounded-lg px-3 py-2.5" style={{ background: involvesMe ? c.surfaceHover : "transparent", border: `1px solid ${involvesMe ? c.borderStrong : c.border}`, opacity: r.confirmed ? 1 : 0.75 }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="font-body text-sm flex items-center gap-1.5 min-w-0">
          ...
          <span className="font-mono text-xs shrink-0" style={{ color: c.textFaint }}>{r.score_one}ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“{r.score_two}</span>
        ...
        <div className="font-mono text-[10px] uppercase tracking-wide" style={{ color: c.textFaint }}>
          {r.kind === "open" ? "Random challenge" : "Challenge"} Ãƒâ€šÃ‚Â· {timeAgo(r.result_confirmed_at)}{!r.confirmed && " Ãƒâ€šÃ‚Â· Awaiting confirmation"}
        </div>
      </div>
    </div>
  );
...
// screen ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â banter, callouts, "who's on tonight" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â open to any signed-in
// member regardless of which challenges they're personally involved in.
// Threads nest to unlimited depth, same as the per-league comments system ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
export function ChallengeBoard({ session, comments, isAdmin, myUsername, onPost, onDelete, onToggleReaction, c, heading = "Challenge board", emptyText = "No comments yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â say something to get things going." }) {
  const [text, setText] = useState("");
  const [posting, setPosting] = useState(false);
        ...
        <>
          ...
          {voiceRecorder.state === "denied" && (
            <div className="font-mono text-[10px] mb-2 ml-10" style={{ color: c.red }}>
              Couldn't access your microphone ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â check your browser's permissions.
            </div>
          )}
          <div className="flex items-end gap-2">
            ...
            <textarea ref={textareaRef} value={text}
              ...
              onKeyDown={onKeyDown}
              placeholder="Say somethingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" rows={1} maxLength={1000}
...
// A single comment on the challenge board, its reaction/reply row, and ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
// recursively ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â every reply underneath it, no matter how deep. Each node
      ...
      <div className="flex items-start gap-2.5 group" style={{ marginLeft: indent }}>
        ...
        <div className="flex-1 min-w-0 rounded-xl px-3 py-2 transition-colors" style={{ background: isSpeaking ? c.surfaceHover : c.surface }}>
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="font-mono text-[10px]" style={{ color: c.textFaint }}>
                {cm.pending ? "sendingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : timeAgo(cm.created_at)}
              </span>
              {!cm.pending && cm.body && (
              ...
              <div className="relative" ref={pickerRef}>
                <button onClick={handleMainClick} disabled={!session}
                  ...
                  <span key={popKey} className={popKey > 0 ? "board-react-pop" : ""} style={{ fontSize: 12, lineHeight: 1 }}>
                    {myReaction ? REACTION_EMOJI[myReaction] : "ÃƒÂ°Ã…Â¸Ã‚Â¤Ã‚Â"}
                  </span>
                  {reactions.length > 0 && (
        ...
        <div className="mt-2" style={{ marginLeft: indent + 38 }}>
          ...
          <div className="flex items-end gap-2">
            <textarea ref={replyRef} value={replyText}
              ...
              placeholder={`Reply to ${cm.username}ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦`} rows={1} maxLength={1000} autoFocus
...
// One grabbable row in the open-challenge pool ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â anyone but the creator can
...
export function ChallengeRow({ challenge: ch, myId, myUsername, onAccept, onDecline, onRemove, onOpenLogResult, onConfirmResult, onDisputeResult, onViewResultProof, onOpenChat, c }) {
  ...
  // Scores are stored from the challenger's perspective ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â flip them for
    ...
    <div className="rounded-xl p-3.5 border" style={{ background: c.surface, borderColor: c.border }}>
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {ch.status === "accepted" && ch.result_status === "confirmed" && (
            <div className="font-mono text-[10px] uppercase tracking-wide flex items-center gap-1" style={{ color: c.greenText }}>
              Final: you {myScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ {theirScore} {counterpartUsername}
              {ch.auto_verified && <span title="Screenshot verified automatically">Ãƒâ€šÃ‚Â· auto-approved</span>}
            </div>
          )}
          {ch.status === "accepted" && ch.result_status === "pending" && iReported && !challengeResultConfirmExpired(ch) && (
            <div className="font-mono text-[10px] uppercase tracking-wide flex items-center gap-1" style={{ color: c.textFaint }}><Clock size={10} /> You {myScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ {theirScore} them Ãƒâ€šÃ‚Â· waiting for confirmation</div>
          )}
          {ch.status === "accepted" && ch.result_status === "pending" && !iReported && !challengeResultConfirmExpired(ch) && (
            <div className="font-mono text-[10px] uppercase tracking-wide" style={{ color: c.accent }}>They reported you {myScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ {theirScore} them</div>
          )}
          ...
          {ch.status === "accepted" && ch.result_status === "pending" && challengeResultConfirmExpired(ch) && (
            <div className="font-mono text-[10px] uppercase tracking-wide flex items-center gap-1" style={{ color: c.red }}><Clock size={10} /> You {myScore} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ {theirScore} them Ãƒâ€šÃ‚Â· escalated to admin for review</div>
          )}
          {ch.status === "accepted" && !ch.result_status && <div className="font-mono text-[10px] uppercase tracking-wide" style={{ color: c.greenText }}>Accepted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â say hi and set a time</div>}
          ...
          {ch.status === "expired" && <div className="font-mono text-[10px] uppercase tracking-wide" style={{ color: c.red }}>{iAmChallenger ? "Walkover ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â admin closed it out" : "Expired ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you didn't respond in time"}</div>}
        </div>
        ...
        {ch.status === "accepted" && !ch.result_status && (
          <div className="flex items-center gap-1.5 shrink-0">
            <WhatsAppCallLink phone={counterpartPhone} iconOnly text={`Hi, this is ${myUsername} ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥ Game's on! Call me when you're ready to play so we can lock in the time ÃƒÂ¢Ã…Â¡Ã‚Â½ÃƒÂ°Ã…Â¸Ã¢â‚¬Â¢Ã‚Â¹ÃƒÂ¯Ã‚Â¸Ã‚Â`} c={c} />
...
// Lets two people already matched ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â an accepted direct challenge or a
// grabbed random challenge ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â message each other without leaving the site.
...
export function ChallengeChatModal({ challengeId, kind, myId, counterpartUsername, onClose, showToast, c }) {
  ...
  useEffect(() => {
    ...
    // the handler ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â direct and open challenges never actually share an id
    ...
    const channel = supabase.channel(`challenge-chat-${kind}-${challengeId}`)
      ...
      .subscribe((status, err) => {
        if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
          ...
          showToast?.("Live chat updates aren't connecting ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â try reopening the chat.");
        }
      });
  ...
  const send = async () => {
    ...
    if (error) {
      ...
      setBody(text); // send failed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â put the draft back rather than lose it
    }
  };
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl flex flex-col" style={{ background: c.bg, border: `1px solid ${c.border}`, height: "min(80vh, 640px)" }}>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-2">
          {messages === null ? (
            ...
            <div className="font-body text-xs text-center mt-6" style={{ color: c.textFaint }}>Say hi ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â messages stay right here, no need to leave the site.</div>
          ) : (
            messages.map((m) => {
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-t shrink-0" style={{ borderColor: c.border }}>
          <input value={body} onChange={(e) => setBody(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(); }}
            placeholder="MessageÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" maxLength={1000}
...
function LeagueCard({ league: l, isAdmin, joined, closed, blockedByLeague, myPaymentStatus, canManageLeague, onOpen, onJoin, session, onToggleLeagueReaction, c }) {
  // Ladder Cup never writes to `fixtures` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it plays entirely through
  ...
  return (
    <div onClick={() => onOpen(l.id)} className="group relative shrink-0 w-[168px] rounded-2xl cursor-pointer border overflow-hidden transition-transform active:scale-[0.97]"
      ...
      <div className="relative h-[86px] flex items-center justify-center overflow-hidden"
        ...
        {isLadderCup ? (
          // No fixed match count to show a progress bar against ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Ladder
      ...
      <div className="p-2.5">
        ...
        <div className="flex items-center gap-1 mt-2 font-mono text-[9px]" style={{ color: c.textDim }}>
          ...
          {isLadderCup
            ? ladderMatches.length > 0 && <span className="ml-1">Ãƒâ€šÃ‚Â· {played} played</span>
            : l.fixtures.length > 0 && <span className="ml-1">Ãƒâ€šÃ‚Â· {played}/{l.fixtures.length}</span>}
        </div>
        ...
        <div className="mt-2">
          {joined ? (
            ...
            <span title={`Active in "${blockedByLeague.name}" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â finish or get eliminated there first`}
...
export function StandingsPanel({ standings, zoneFor, stageFixtures, isSurvivor, league, avatarByTeamId, session, myTeamId, c }) {
  ...
  // one match actually being played/expired in the stage ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â with 0 played,
    ...
    <div className="-mx-4 px-4">
      <div className="flex items-center justify-between gap-3 mb-3 px-2">
        <div className="font-mono text-xs" style={{ color: c.textFaint }}>
          ...
          {isSurvivor ? ` Ãƒâ€šÃ‚Â· ${league.final_stage_started ? "final stage" : `stage ${league.current_stage}`}` : ""}
        </div>
        <div className="flex items-center gap-2 shrink-0">
      ...
      {scrolls && (
        <div className="font-mono text-[10px] text-center mt-2" style={{ color: c.textFaint }}>Scroll for more ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â showing {STANDINGS_VISIBLE_ROWS} of {filtered.length}</div>
      )}
      ...
      {profileRow && (
        <PlayerProfileModal
          ...
          stats={[
            ...
            { label: "W Ãƒâ€šÃ‚Â· D Ãƒâ€šÃ‚Â· L", value: `${profileRow.w} Ãƒâ€šÃ‚Â· ${profileRow.d} Ãƒâ€šÃ‚Â· ${profileRow.l}` },
...
// Save button (disabled until a photo is attached ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same proof requirement as
// regular players). A joined non-manager instead gets a "Submit result" button
// that opens the photo + score modal ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â their result lands as pending until an
...
// "Find yourself" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â each icon calls the OTHER team's number and is signed
...
function FixtureScoreRow({ fixture, homeTeam, awayTeam, canManage, onSave, legLabel, joined, submission, onOpenSubmitResult, showContact, hideDueDate, league, c }) {
  ...
  const [photo, setPhoto] = useState(null); // photo proof, required before saving ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same rule as regular players
  ...
  // The final is always a single decisive match ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a level scoreline here
  ...
  const callText = (fromTeam) =>
    `Hi, it's ${fromTeam.name} ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥ Call me when you're ready to play ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â matchday ${fixture.round} is due ${fmtDate(fixture.due_at)}, let's lock in the time ÃƒÂ¢Ã…Â¡Ã‚Â½ÃƒÂ°Ã…Â¸Ã¢â‚¬Â¢Ã‚Â¹ÃƒÂ¯Ã‚Â¸Ã‚Â${firstMatchdayNote(fixture.round)}`;
    ...
    <div className="flex flex-wrap items-center gap-2 py-2">
      ...
      {canManage ? (
        <>
          ...
          <span className="shrink-0" style={{ color: c.textFaint }}>ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“</span>
        ...
        <span className="font-mono text-sm w-14 text-center shrink-0" style={{ color: c.text }}>
          {fixture.played ? `${fixture.home_score} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${fixture.away_score}` : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ : ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“"}
      ...
      {/* For a two-legged tie, both legs now share one due_at ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â showing it
          on every row would just repeat the same date twice. The shared
          startÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“expiry window is shown once instead, at the tie level (see
          KnockoutFixturesList) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this column is skipped here via
      ...
      {canManage && (
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          ...
          <button onClick={save} disabled={saveState === "saving" || !photo || !pensReady} title={!photo ? "Attach a photo proof to save" : !pensReady ? "Enter a decisive penalty score" : undefined}
            ...
            {saveState === "saved" ? <Check size={12} /> : saveState === "saving" ? "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "Save"}
          </button>
        </div>
      )}
                ...
                <div key={r} className="px-4 py-2.5">
                  ...
                  <div className="divide-y" style={{ borderColor: c.border }}>
                    {roundsMap[r].map((f) => {
                      ...
                      if (!away) {
                        return <div key={f.id} className="py-2 font-body text-xs" style={{ color: c.textFaint }}>{home?.name} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â bye this round</div>;
                      }
            ...
            <div className="rounded-xl border divide-y" style={{ borderColor: c.border, background: c.surface }}>
              {ties.map((legs) => {
                ...
                if (f0.away_team_id === null) {
                  return <div key={f0.id} className="px-4 py-2.5 font-body text-xs" style={{ color: c.textFaint }}>{home?.name} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â bye, advances automatically</div>;
                }
                ...
                // "start ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ expiry (N days)" range instead of two separate
                  ...
                  <div key={f0.id} className="px-4 py-2.5">
                    {twoLegged && !allPlayed && (
                      <div className="font-mono text-[10px] mb-1.5" style={{ color: tieExpired ? c.red : c.textDim }}>
                        {tieExpired
                          ? "Expired"
                          : `${fmtDate(tieStartAt)} ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ ${fmtDate(tieDueAt)} (${tieWindowDays} day${tieWindowDays === 1 ? "" : "s"})`}
                      </div>
                    )}
                    {legs.map((f) => {
                    ...
                    {(twoLegged || level) && (
                      <div className="font-mono text-[10px] mt-1" style={{ color: c.textDim }}>
                        {twoLegged && <>Aggregate: {home?.name} {hAgg} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ {aAgg} {away?.name}</>}
                        {level && isFinalTie && pensH !== null && pensA !== null && pensH !== pensA && (
                          <span style={{ color: c.textDim }}> Ãƒâ€šÃ‚Â· pens {pensH}-{pensA}</span>
                        )}
                        {level && isFinalTie && !(pensH !== null && pensA !== null && pensH !== pensA) && (
                          <span style={{ color: c.red }}> Ãƒâ€šÃ‚Â· level ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â needs a penalty shootout score to decide the winner</span>
                        )}
                        {level && !isFinalTie && (
                          <span style={{ color: c.greenText }}> Ãƒâ€šÃ‚Â· level on aggregate ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â both clubs advance</span>
                        )}
                      </div>
                    )}
                  </div>
        ...
        return (
          <div key={g}>
            <div className="font-mono text-xs uppercase tracking-[0.2em] mb-2 flex items-center gap-2" style={{ color: c.textFaint }}>
              {groupLabel(g)}
              {qualifiers > 0 && n > 0 && (
                <span className="normal-case font-body text-[11px]" style={{ color: c.greenText }}>Ãƒâ€šÃ‚Â· top {Math.min(qualifiers, n)} advance</span>
              )}
            </div>
      ...
      {canManage && (
        <>
          ...
          <button onClick={() => inputRef.current?.click()} disabled={uploading}
            ...
            <Settings2 size={12} /> {uploading ? "UploadingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : league.photo_url ? "Change photo" : "Add photo"}
          </button>
        </>
      )}
    </div>
...
export function LeagueScheduleLine({ league, canManage, onUpdateSchedule, onUpdateRoundPeriod, c }) {
  ...
  // LadderCupPendingPanel), not a generic registration window ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so this
  ...
  const [saving, setSaving] = useState(false);
  const inputStyle = { background: c.surfaceHover, borderColor: c.border, color: c.text };
  // Fixtures only exist once the admin has started the league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the due-date
  ...
  if (editing) {
    return (
      <div className="mt-2 rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
        ...
        {isLadderCup && (
          <div className="font-mono text-[11px] mb-2" style={{ color: c.textFaint }}>Clubs can join anytime ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Survival Ladder Cup has no entry-close date, only its own weekly cutoff.</div>
        )}
        {datesOutOfOrder && (
        ...
        {notStartedYet ? (
          ...
          <div className="font-mono text-[11px] mb-1.5" style={{ color: c.textFaint }}>
            Match due-date period ({league.round_period_hours || DEFAULT_ROUND_PERIOD_HOURS} hour{(league.round_period_hours || DEFAULT_ROUND_PERIOD_HOURS) === 1 ? "" : "s"}) is locked in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the league has already started.
          </div>
        )}
  ...
  return (
    <div className="flex items-center flex-wrap gap-x-1.5 gap-y-1 mt-1">
      <div className="font-mono text-[11px] flex items-center gap-1.5" style={{ color: c.textFaint }}>
        <Clock size={11} /> {isLadderCup ? `${league.ladder_cup_started_at ? "Started" : "Open for joining"} Ãƒâ€šÃ‚Â· Starts ${fmtDate(league.starts_at)}` : `Entry closes ${fmtDate(league.entry_closes_at)} Ãƒâ€šÃ‚Â· Starts ${fmtDate(league.starts_at)}`}
      </div>
      {canManage && (
...
// matchday's own due_at stays purely advisory once this exists ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â it's still
    ...
    return (
      <div className="mt-2 rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
        ...
        <div className="font-mono text-[11px] mt-1.5 mb-2" style={{ color: c.textFaint }}>
          Individual matchday due dates stay advisory ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this is the real cutoff for the whole group stage.
        </div>
        <div className="flex items-center gap-2 justify-end">
  ...
  return (
    <div className="flex items-center flex-wrap gap-x-1.5 gap-y-1 mt-1">
      <div className="font-mono text-[11px] flex items-center gap-1.5" style={{ color: passed ? c.red : c.textFaint }}>
        <Clock size={11} />
        {league.group_stage_due_at ? `Group stage due ${fmtDate(league.group_stage_due_at)}${passed ? " Ãƒâ€šÃ‚Â· expired" : ""}` : "Group stage due date not set"}
      </div>
      {canManage && (
        <button onClick={() => setEditing(true)} className="flex items-center gap-1 font-mono text-[11px] font-semibold px-1.5 py-0.5 -my-0.5 rounded"
          style={{ color: c.accent }}>
        ...
        editing ? (
          <div className="mt-2 rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
            <textarea value={text} onChange={(e) => setText(e.target.value)} rows={4}
              placeholder="Rules, prize info, WhatsApp group link ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â anything players should know."
            ...
            <div className="flex items-center gap-2 justify-end">
              ...
              <button onClick={save} disabled={saving} className="font-body text-xs font-semibold px-3 py-1.5 rounded-full" style={{ background: c.accent, color: c.accentText, opacity: saving ? 0.6 : 1 }}>{saving ? "SavingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "Save"}</button>
            </div>
          </div>
        ) : (
...
// auto-generated WhatsApp nudge text ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see adminStatusMessage ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â with their
...
// everyone. Admin-only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this is an internal tool for whoever's sending
...
export function MemberMessageEditor({ league, onUpdateMemberMessage, onNotifyAllMembers, c }) {
  ...
  // reads like an actual message rather than a placeholder ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â falls back to
  ...
  return (
    <div className="rounded-xl p-4 mb-3 border" style={{ background: c.surface, borderColor: c.border }}>
      <div className="font-mono text-[11px] uppercase tracking-wide mb-2" style={{ color: c.textDim }}>
        Sent to every member's WhatsApp icon in this league ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use <strong>{"{name}"}</strong> for their name, <strong>{"{league}"}</strong> for the league name,
      ...
      <textarea value={text} onChange={(e) => setText(e.target.value.slice(0, MAX_LEN))} rows={4} maxLength={MAX_LEN}
        placeholder="Hey {name}! Round {round} of {league} is due {due} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â lock it in! ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥ÃƒÂ¢Ã…Â¡Ã‚Â½"
      ...
      {preview && (
        <div className="rounded-lg px-3 py-2 mb-2 font-body text-xs whitespace-pre-wrap" style={{ background: c.surfaceHover, color: c.textDim }}>
          <span className="font-mono text-[10px] uppercase tracking-wide block mb-1" style={{ color: c.textFaint }}>
            Preview ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â as {sampleName} would see it
          </span>
          {preview}
        </div>
      )}
...
// One row for a joined member ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â team, and (for cash leagues) payment status with
          ...
          return (
            <div key={s.id} className="rounded-lg px-4 py-2.5" style={{ background: c.surface }}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-body text-sm truncate">{home?.name || "Home"} {s.home_score} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ {s.away_score} {away?.name || "Away"}</div>
                  <div className="font-mono text-[11px]" style={{ color: c.textFaint }}>Submitted by {s.submitted_by_username}{fixture ? ` Ãƒâ€šÃ‚Â· Matchday ${fixture.round}` : ""} Ãƒâ€šÃ‚Â· {timeAgo(s.created_at)}</div>
                  {showDeadline && (() => {
                    ...
                    return (
                      <div className="font-mono text-[11px] mt-0.5" style={{ color: reason ? c.red : (resultConfirmMinutesLeft(s, league) <= 5 ? c.red : "#B8860B") }}>
                        {reason === "dispute-cap"
                          ? "This fixture's been disputed too many times already ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â sent straight to the admin"
                          : reason === "timeout"
                          ? "Confirmation window passed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this has been sent to the admin"
                          : `${resultConfirmMinutesLeft(s, league)}m left to respond ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â after that it goes to the admin`}
                      </div>
                    );
                  })()}
                  {!showDeadline && showEscalationReason && resultEscalationReason(league, s) === "dispute-cap" && (
                    <div className="font-mono text-[11px] mt-0.5" style={{ color: c.red }}>Escalated ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this fixture's been disputed too many times already</div>
                  )}
                </div>
              </div>
...
// automatically to the next unplayed fixture's due date ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the viewer's own
...
// the next fixture due date. Kept upbeat on purpose ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â this is the message
// that lands in a player's WhatsApp, not a formal notice.
// Whether THIS member would actually get the league's saved custom
// template, or fall back to the automated status message ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â shared by
...
// them ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â for a member with none (eliminated, or nothing left to play),
...
// The real kickoff moment for a fixture ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â when players should actually
...
// period ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â same fallback logic already used for two-legged knockout ties
...
function adminStatusMessage(m, t, league) {
  ...
  // above), until it's edited or cleared again ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see
  ...
  // {due} are also live ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â sourced from this member's own next unplayed
  ...
  // real kickoff moment (see fixtureStartsAt) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â blank if there's no
  ...
  if (t?.eliminated) {
    return `Hey ${name}! ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Â¹\nÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â´ Tough one ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you've been eliminated from ${league.name}.\nÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥ Try again on the next one ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â jump into one of our other available leagues and get straight back in the fight!\nÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Â° ${SITE_URL}`;
  }
  const notStarted = league.fixtures.length === 0;
  if (notStarted) {
    return league.starts_at
      ? `Hey ${name}! ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Â°\nÃƒÂ°Ã…Â¸Ã‚ÂÃ¢â‚¬Â  ${league.name} kicks off ${fmtDate(league.starts_at)}.\nÃƒÂ¢Ã…Â¡Ã‚Â½ Get ready, it's going to be a good one!`
      : `Hey ${name}! ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Â°\nÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ ${league.name} is filling up fast.\nÃƒÂ¢Ã…Â¡Ã‚Â½ We'll confirm the kickoff date soon ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â get hyped!`;
  }
  ...
  const upcoming = t ? nextFixtureForTeam(league, t.id) : null;
  if (upcoming) {
    // The window this fixture can be played in ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â real kickoff moment
    ...
    const windowLine = start && start !== upcoming.due_at
      ? `ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¦ Starts ${fmtDate(start)} Ãƒâ€šÃ‚Â· Due ${fmtDate(upcoming.due_at)}`
      : `ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¦ Due ${fmtDate(upcoming.due_at)}`;
    // Round 1 of a fresh stage means this club just survived a cut ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the
    // knockout bracket starting for groups_knockout, or a new survivor
    // stage (current_stage > 1) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so lead with a congrats line instead of
    ...
    //
    // A plain knockout league has no earlier stage to be promoted FROM ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
    ...
    if (justAdvanced) {
      const throughTo = league.format === "knockout" ? "the next round"
        ...
        : "the knockout stage";
      return `Hey ${name}! ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Â°\nÃƒÂ°Ã…Â¸Ã‚ÂÃ¢â‚¬Â  Congrats ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you're through to ${throughTo} of ${league.name}!\nÃƒÂ°Ã…Â¸Ã‚ÂÃ…Â¸ÃƒÂ¯Ã‚Â¸Ã‚Â Round ${upcoming.round} is up next.\n${windowLine} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â lock in a time with your opponent.\nÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥ Bring the heat!\nÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Â° ${SITE_URL}`;
    }
    return `Hey ${name}! ÃƒÂ¢Ã…Â¡Ã‚Â¡\nÃƒÂ°Ã…Â¸Ã‚ÂÃ…Â¸ÃƒÂ¯Ã‚Â¸Ã‚Â Round ${upcoming.round} in ${league.name} is up next.\n${windowLine} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â lock in a time with your opponent.\nÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¥ Bring the heat!${firstMatchdayNote(upcoming.round)}`;
  }
  return `Hey ${name}! ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Â¹\nÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ This is weAfrica admin Saul, checking in on ${league.name}.`;
}
...
// regardless of fixtures, due dates, or elimination status ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â purely "was
...
// Contribution ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ direct prize ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ redistributed ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ total balance, for every
...
export function PrizeBreakdownPanel({ league, c }) {
  ...
  const medal = (rank) => (rank === 1 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â¡ " : rank === 2 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã‹â€  " : rank === 3 ? "ÃƒÂ°Ã…Â¸Ã‚Â¥Ã¢â‚¬Â° " : `#${rank} `);
  ...
  return (
    <div className="rounded-xl border mt-4" style={{ borderColor: c.border }}>
      ...
      <div className="px-4 pb-3 font-mono text-[11px]" style={{ color: c.textFaint }}>
        {knockoutFormat
          ? `Pool ${formatRand(pool)} Ãƒâ€šÃ‚Â· 75% champion Ãƒâ€šÃ‚Â· 20% runner-up Ãƒâ€šÃ‚Â· 5% organizer fee`
          : `Pool ${formatRand(pool)} Ãƒâ€šÃ‚Â· 55% gold Ãƒâ€šÃ‚Â· 25% silver Ãƒâ€šÃ‚Â· 15% bronze Ãƒâ€šÃ‚Â· 5% organizer fee`}
        {!complete ? " Ãƒâ€šÃ‚Â· updates live as results come in" : ""}
      </div>
          ...
          <tbody>
            {rows.map(({ m, prize }) => (
              <tr key={m.id} className="border-t" style={{ borderColor: c.border }}>
                ...
                <td className="text-right px-2 py-2">{prize ? formatRand(Math.round(prize.directPrize)) : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td>
                <td className="text-right px-2 py-2">{prize ? formatRand(Math.round(prize.redistributed)) : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td>
            ...
            {orgFee > 0 && (
              <tr className="border-t" style={{ borderColor: c.border }}>
                ...
                <td className="text-right px-2 py-2" style={{ color: c.textFaint }}>ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â</td>
                <td className="text-right px-2 py-2" style={{ color: c.textFaint }}>ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â</td>
                <td className="text-right px-2 py-2" style={{ color: c.textFaint }}>ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â</td>
...
// Kebab menu on the league page for admin/creator actions ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â keeps "Delete league"
