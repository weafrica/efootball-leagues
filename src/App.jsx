...
// place it directly after a phone number without an extra guard.
function WhatsAppLink({ phone, text, label, c }) {
          ...
          <>
            ...
            {view === "league" && activeLeague && (
              <LeagueDetail league={activeLeague} session={session} isAdmin={isAdmin} joined={isMemberOf(activeLeague)}
                ...
                onDelete={deleteLeague} onShare={shareLeague} onLeave={leaveLeague} c={c} />
            )}
          </>
        )}
      </main>
  ...
  return (
    <div className="rounded-lg px-4 py-2.5 flex items-center gap-3" style={{ background: c.surface }}>
      ...
      {!isFlagged && digitsOnly && (
        <a href={waLink(account.phone, `Hi ${account.efootball_username || "there"}, this is the Matchday admin.`)} target="_blank" rel="noopener noreferrer" title="Message on WhatsApp"
...
function LeagueDetail({ league, session, isAdmin, joined, canSeePhones, myTeam, entryClosed, myPaymentStatus, onBack, onJoin, onResubmitPayment, onDownloadProof, onReviewPayment, onRecordResult, onUpdateTeamPhone, onRemoveTeam, onUpdatePhoto, onUpdateDescription, onAdvance, onGenerateFixtures, onDelete, onShare, onLeave, c }) {
    ...
    <div className="pt-8">
      ...
      {tab === "fixtures" && (
        <div className="space-y-6">
          ...
          {canSeePhones && <TeamContactsPanel teams={league.teams} leagueName={league.name} canManage={canManage} onUpdateTeamPhone={onUpdateTeamPhone} c={c} />}
          {joined && !canSeePhones && (
...
function TeamContactsPanel({ teams, leagueName, canManage, onUpdateTeamPhone, c }) {
  const [query, setQuery] = useState("");
    ...
    <div className="rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
      ...
      <div className="space-y-2 max-h-72 overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          <TeamContactRow key={t.id} team={t} leagueName={leagueName} canManage={canManage} onUpdateTeamPhone={onUpdateTeamPhone} c={c} />
        ))}
      </div>
    </div>
  );
...
function TeamContactRow({ team, leagueName, canManage, onUpdateTeamPhone, c }) {
  const [editing, setEditing] = useState(false);
  ...
  const message = canManage
    ? `Hi ${team.name}, this is the ${leagueName || "league"} admin — reaching out about your matches.`
                ...
                {canSeePhones && (
                  opp.opponent?.phone ? (
                    <div className="flex items-center gap-2 mt-1">
                      <span className="font-mono text-xs" style={{ color: c.greenText }}>{opp.opponent.phone}</span>
                      <WhatsAppLink phone={opp.opponent.phone} text={`Hi, it's ${result.team.name} — ready to set up our matchday?`} c={c} />
                    </div>
        ...
        <div className="font-body text-sm mt-3 rounded-lg px-3 py-2.5" style={{ background: c.surfaceHover }}>
          ...
          {canSeePhones ? (
            result.opponent.phone ? (
              <div className="flex items-center gap-2 mt-1">
                <span className="font-mono text-xs" style={{ color: c.greenText }}>{result.opponent.phone}</span>
                <WhatsAppLink phone={result.opponent.phone} text={`Hi, it's ${result.team.name} — matchday ${matchday} clash, when works for you?`} c={c} />
              </div>
            ) : <div className="font-mono text-xs mt-1" style={{ color: c.textFaint }}>No number on file for this club yet.</div>
          ) : (
