    ...
    <div className="pt-8">
      ...
      {tab === "fixtures" && (
        <div className="space-y-6">
          {joined && <OpponentFinder league={league} totalRounds={totalRounds} c={c} />}
          {joined && <TeamContactsPanel league={league} onUpdateTeamPhone={onUpdateTeamPhone} c={c} />}
          {Array.from({ length: totalRounds }, (_, i) => i + 1).map((round) => (
            <div key={round}>
              <div className="font-mono text-xs uppercase tracking-[0.2em] mb-2" style={{ color: c.textFaint }}>Matchday {round}</div>
...
function TeamContactsPanel({ league, onUpdateTeamPhone, c }) {
  ...
  const filtered = query.trim()
    ? league.teams.filter((t) => t.name.toLowerCase().includes(query.trim().toLowerCase()))
    : league.teams;
  return (
    <div className="rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
      ...
      <input
        ...
        className="w-full border rounded-lg px-3 py-2 font-body text-sm outline-none mb-3"
...
function OpponentFinder({ league, totalRounds, c }) {
  ...
  const search = () => {
    ...
    if (!md || md < 1 || md > totalRounds) {
      setResult({ notFound: true, reason: `Enter a matchday between 1 and ${totalRounds}.` });
      return;
    }
    const team = league.teams.find((t) => t.name.trim().toLowerCase() === teamQuery.trim().toLowerCase());
    if (!team) {
      setResult({ notFound: true, reason: "No club with that exact name — pick one from the suggestions." });
      return;
    }
    const fixture = league.fixtures.find((f) => f.round === md && (f.home_team_id === team.id || f.away_team_id === team.id));
    if (!fixture) {
      setResult({ notFound: true, reason: `${team.name} has no fixture on matchday ${md}.` });
      return;
    }
    ...
    setResult({
      ...
      score: fixture.played ? `${fixture.home_score} – ${fixture.away_score}` : null,
    });
  };
  ...
  return (
    <div className="rounded-xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          type="number" min={1} max={totalRounds} value={matchday}
          onChange={(e) => setMatchday(e.target.value)}
          placeholder="Matchday #"
          className="w-full sm:w-32 border rounded-lg px-3 py-2 font-mono text-sm outline-none"
          style={{ background: c.surfaceHover, borderColor: c.border, color: c.text }}
        />
        <input
          list="team-names-datalist" value={teamQuery} onChange={(e) => setTeamQuery(e.target.value)}
      ...
      {result && (
        result.notFound ? (
          ...
          <div className="font-body text-sm mt-3 rounded-lg px-3 py-2.5" style={{ background: c.surfaceHover }}>
            <div className="font-semibold">{result.opponent} <span className="font-mono text-xs font-normal" style={{ color: c.textFaint }}>({result.venue})</span></div>
            {result.played && <div className="font-mono text-xs mt-1" style={{ color: c.textDim }}>Final score: {result.score}</div>}
            {result.phone ? (
              <div className="font-mono text-xs mt-1" style={{ color: c.greenText }}>{result.phone}</div>
            ) : (
