...
//     desc: "Ranked ladder, one elimination life each. Most points by the month-end cutoff wins.",
//     available: true }
//
...
/**
 * Both "buy your life back" prices — the second-life points deduction and
 * the rebirth Nets fee — scale with how many wins a club is carrying: +10%
 * per win, rounded at each step rather than compounded once and rounded at
 * the end (so it lands the same way someone manually recomputing "add 10%,
 * round" win-by-win would get). wins = 0 returns baseFee unchanged.
 *
 * Deliberately computed from the entry's CURRENT w every time this is
 * called (at accept-time / rebirth-time), never snapshotted onto the offer
 * when it's first made. That's what makes it apply to an offer that's
 * already sitting in pending_second_life right now, same as a brand-new
 * one — there's no separate "old offers keep the old price" state to
 * migrate, because nothing about the price was ever stored on the offer
 * itself.
 */
export function winScaledFee(baseFee, wins) {
  let amount = baseFee;
  for (let i = 0; i < (wins || 0); i++) {
...
export function acceptSecondLife(entry) {
  ...
  const deduction = winScaledFee(LADDER_CUP_RULES.SECOND_LIFE_DEDUCTION, entry.w);
  return {
    ...entry,
    pts: Math.max(0, entry.pts - deduction),
    status: "active",
...
// club can choose to rejoin.
//
// pts/w/l/gd/streak carry straight through a rebirth — buying your life
// back costs LADDER_CUP_REBIRTH_FEE_NETS, not your standing. A life-number
// checkpoint of where you stood the moment you bought it back is still
// archived into `past_lives` (display-only — see careerLadderCupTotals
// below; nothing in this file ever adds it a second time on top of
// entry.pts) purely so the UI can show "life 2 started at 340 pts" — it's
// never subtracted from or reset back into the live row. Only the things
// that gated your elimination reset: second life (a fresh one becomes
// available again), ladder_rating (so matchmaking isn't anchored to the
// run that just ended), badges, and status back to "active".
...
/**
 * Records a life-checkpoint into past_lives (pts/w/l/gd as they stood the
 * moment the life was bought back — for "life 2 started at 340 pts"-style
 * history, not a stat reset) and clears elimination: a fresh second life,
 * ladder_rating back to the starting value, badges cleared, status back to
 * "active". pts/w/l/gd/streak are untouched — rebirth_count increments so
 * the UI can badge "2nd life", "3rd life", etc.
 */
export function reborn(entry, now = new Date()) {
  ...
  return {
    ...
    past_lives: [...(entry.past_lives || []), lifeCheckpoint],
    reborn_at: now.toISOString(),
  };
}

/**
 * Career totals for a club profile's "all-time" line. pts/w/l/gd carry
 * through rebirths now (see reborn() above), so entry.pts/w/l already ARE
 * the full career total — past_lives is a list of checkpoints along that
 * same continuous run, not separate finished totals to add on top of it.
 */
...
const REBIRTH_ANNOUNCEMENTS = [
  (club, life) => `🔥 RISEN FROM THE ASHES! ${club} has been REBORN into the Survival Ladder Cup — every one of their ${life.pts} pts (${life.w}W-${life.l}L across ${life.matches_played} matches) comes with them. Eliminated, but never reset.`,
  (club, life) => `⚡ ${club} REFUSES TO STAY DOWN. Eliminated, but never forgotten — that ${life.w}-win run stays on the record, and the ${life.pts} pts behind it stay right where they were. Back on the ladder, still standing on everything they built.`,
  (club, life) => `👑 A PHOENIX MOMENT for ${club}! Struck down after ${life.matches_played} battles, they've chosen rebirth — and kept every one of their ${life.pts} pts (${life.w}W-${life.l}L) doing it. Same fire, same standing.`,
];
...
/**
 * Filters out anything not finalized by the month-end (last day of the
 * month, 23:59 UTC) cutoff: matches still mid-play, and walkover claims
 * still awaiting admin review (if they weren't approved before the
 * cutoff, they don't count).
 *
