...
import {
  ...
  MoreVertical, Send, CornerDownRight, Camera, Eye, ThumbsUp, ThumbsDown, Target, ChevronDown, History,
} from "lucide-react";
...
function computeRecentMatches(leagues, bounds) {
  ...
  (leagues || []).forEach((l) => {
    (l.fixtures || []).forEach((f) => {
      ...
      if (!home || !away) return;
      rows.push({ id: f.id, leagueName: l.name, homeName: home.name, awayName: away.name, homeScore: f.home_score, awayScore: f.away_score, playedAt: at });
    });
  });
            ...
            {pastMatches.slice(0, 40).map((m) => (
              <div key={m.id} className="flex items-center justify-between gap-3 rounded-lg px-4 py-2.5" style={{ background: c.surface }}>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="font-mono text-[10px] truncate" style={{ color: c.textFaint }}>{m.leagueName} · {fmtDate(m.playedAt)}</div>
                </div>
...
function Home({ leagues, isAdmin, isMemberOf, entryClosed, myPaymentStatus, canManageLeague, onOpen, onCreate, onJoin, session, onToggleLeagueReaction, c }) {
  ...
  return (
    <div>
      <section className="pt-8 pb-8" style={{ borderBottom: `1px solid ${c.border}` }}>
