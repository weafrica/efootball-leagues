      ...
      <div className="relative rounded-2xl pl-5 pr-4 py-4 mb-4 overflow-hidden"
        ...
        <div className="relative flex items-center justify-between gap-2 mt-3 pt-3" style={{ borderTop: `1px solid ${c.border}` }}>
          <div className="leading-tight">
            ...
            <div className="font-mono text-sm font-bold">{netsBalance === null ? "…" : <NetsAmount amount={netsBalance} size={14} />}</div>
          </div>
            ...
            <div key={l.id} className="rounded-2xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
              <div className="flex items-start justify-between gap-2">
                ...
                <p className="font-mono font-bold text-sm shrink-0" style={{ color: KIT_ROOM_COBALT }}>
                  {l.asking_price ? <NetsAmount amount={l.asking_price} /> : "Offers only"}
                </p>
              </div>
            ...
            return (
              <div key={l.id} className="rounded-2xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
                ...
                <p className="font-mono font-bold text-sm mt-2" style={{ color: KIT_ROOM_COBALT }}>
                  {l.status === "sold" ? <>Sold for <NetsAmount amount={l.sold_price} /></> : (l.asking_price ? <NetsAmount amount={l.asking_price} /> : "Offers only")}
                </p>
                {l.status === "active" && (
                    ...
                    {offers === undefined ? <Spinner c={c} /> : offers.length === 0 ? (
                      ...
                      <div key={o.id} className="rounded-xl p-3" style={{ background: c.surfaceHover }}>
                        <div className="flex items-center justify-between">
                          <p className="font-mono font-bold text-sm" style={{ color: c.text }}><NetsAmount amount={o.amount} /></p>
          ...
          {myOffers.map((o) => {
            ...
            return (
              <div key={o.id} className="rounded-2xl p-4 border" style={{ background: c.surface, borderColor: c.border }}>
                ...
                <p className="font-mono font-bold text-sm mt-2" style={{ color: KIT_ROOM_COBALT }}>Your offer: <NetsAmount amount={o.amount} /></p>
                {o.status === "pending" && (
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4" style={{ background: "rgba(0,0,0,0.65)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-5" style={{ background: c.bg, border: `1px solid ${c.border}` }}>
        ...
        {listing.asking_price && (
          <p className="font-body text-xs mb-3" style={{ color: c.textDim }}>Asking price: <NetsAmount amount={listing.asking_price} /></p>
        )}
...
// Blob under "team-sale-photos/" via the same uploadToR2() helper every
...
function TeamSaleSellForm({ onSubmit, c, showToast, session }) {
  ...
  const pickPhoto = async (slotKey, file) => {
    ...
    try {
      ...
      const url = await uploadToR2("team-sale-photos", path, compressed, compressed.type || "image/jpeg");
