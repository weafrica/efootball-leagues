...
function Header({ view, setView, activeLeague, theme, toggleTheme, c, onSignOut, userEmail, onEditProfile, isAdmin, onOpenAccounts, onOpenSuggestion }) {
  return (
...
function LeagueReactionBar({ league, session, onToggle, c }) {
  ...
  return (
    <div className="relative mb-5" ref={pickerRef}>
      <button onClick={handleMainClick} disabled={!session}
        className="flex items-center gap-1.5 font-mono text-[11px] px-2.5 py-1.5 rounded-full transition-colors"
        style={{ background: c.surface, color: myReaction ? c.accent : c.textFaint }}>
        <span style={{ fontSize: 13, lineHeight: 1 }}>{myReaction ? REACTION_EMOJI[myReaction] : "🤍"}</span>
        {myReaction ? "You reacted" : "React to this league"}
        {reactions.length > 0 && (
      ...
      {pickerOpen && (
        <div className="reaction-picker absolute top-full left-0 mt-1.5 flex items-center gap-0.5 rounded-full px-1.5 py-1 shadow-lg z-10"
