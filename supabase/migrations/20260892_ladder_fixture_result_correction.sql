-- League Ladder — result approval: correcting an already-played fixture.
--
-- Once a fixture is status='played' (confirmed by the opponent, an admin,
-- or the auto-approve sweep), its score can't just be overwritten in
-- place — promotion/relegation, fee settlement, decay penalty, and streaks
-- may already have run against it for that week. Mirrors
-- correct_ladder_cup_match_result (20260822): log the before/after to
-- ladder_fixture_corrections (20260887), rewrite the raw score, then — if
-- that fixture's week has already closed — re-run the weekly resolve so
-- standings/promotion/fees pick up the corrected score.
--
-- Unlike Ladder Cup, this needs no bulk-replay engine of its own: League
-- Ladder's weekly resolve (_ladder_close_week_internal) already recomputes
-- fresh from ladder_fixtures every time it runs, and
-- admin_retrigger_ladder_resolve (20260864) already exists to call it on
-- demand — this migration just wires a correction into that existing path
