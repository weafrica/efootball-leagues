-- join_rapid_cup_lobby and raise_rapid_cup_entry_fee were reading the
-- player's balance from `balances`/`amount` — an empty, unused legacy
-- table (every real balance lives in nets_wallets/balance; see nets.js's
-- getNetsBalance, which the header and the fee slider already use). That
-- meant the server always computed a 0 Nets cap and rejected any nonzero
-- entry fee with "Entry fee cannot exceed 20% of your Nets balance (max
-- 0)" — regardless of what the client-side slider showed. Routed both
-- functions through nets_wallets so the server's cap agrees with the
-- balance the player actually sees.
